  ...
  const handleWebSocketMessage = useCallback((message: any) => {
    switch (message.type) {
      ...
      case 'SCORE_CHANGE':
        ...
        // Check if this match is relevant to our predictions FIRST
        const relevantPrediction = predictions.find(
          p => p.match_id === message.matchId && p.result === 'pending'
        );

        if (relevantPrediction) {
          console.log('[AIPredictions] 🎯 SCORE_CHANGE for our prediction:', {
            bot: relevantPrediction.canonical_bot_name,
            teams: `${relevantPrediction.home_team_name} vs ${relevantPrediction.away_team_name}`,
            oldScore: `${relevantPrediction.home_score_display}-${relevantPrediction.away_score_display}`,
