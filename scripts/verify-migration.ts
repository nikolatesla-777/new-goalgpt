...
async function verifyMigration() {
  ...
  if (missingXP === 0) {
    console.log('✅ All users have XP records');
  } else {
    console.log(`❌ ${missingXP} users missing XP records`);
    errorCount++;
  }
  ...
  if (missingCredits === 0) {
    ...
    console.log(`❌ ${missingCredits} users missing Credit records`);
    errorCount++;
  }

  // 3. Verify VIP bonus was granted
  console.log('\n🎁 Checking VIP welcome bonuses...');
  ...
  if (vipCount.rows[0].count === vipWithBonus.rows[0].count) {
    console.log('✅ All VIP users received welcome bonus');
  } else {
    console.log('⚠️  VIP bonus count mismatch');
    warningCount++;
  }
  ...
  if (badges >= 3) {
    console.log(`✅ ${badges} badges created (expected >= 3)`);
  } else {
    console.log(`❌ Only ${badges} badges found (expected >= 3)`);
    errorCount++;
  }
  ...
  if (totalUsers.rows[0].count === usersWithReferralCode.rows[0].count) {
    ...
    console.log('⚠️  Some users missing referral codes');
    warningCount++;
  }
  ...
  console.log('\n🔧 Checking altered table columns...');
  const alterChecks = [
  ...
  for (const check of constraintChecks) {
    ...
    if (exists.rows[0].exists) {
      console.log(`   ✅ ${check.table}.${check.constraint}`);
    } else {
      console.log(`   ⚠️  ${check.table}.${check.constraint} missing`);
      warningCount++;
    }
  }
