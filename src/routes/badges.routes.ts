...
interface ClaimBadgeRequest {
  Body: {
    badgeId: string;
  };
}

interface ToggleBadgeDisplayRequest {
  Body: {
    badgeId: string;
    isDisplayed: boolean;
  };
...
export async function badgesRoutes(fastify: FastifyInstance) {
   ...
   */
  fastify.get<GetBadgesRequest>(
    '/',
    async (request, reply) => {
      try {
        const { category, rarity } = request.query;

        const badges = await getAllBadges(category, rarity);
   ...
   */
  fastify.post<UnlockBadgeRequest>(
    '/unlock',
    ...
    async (request, reply) => {
      try {
        ...
        const result = await unlockBadge(userId, badgeSlug, metadata);
  ...
  fastify.post<ToggleBadgeDisplayRequest>(
    ...
    async (request: FastifyRequest<ToggleBadgeDisplayRequest>, reply: FastifyReply) => {
      try {
        ...
        const result = await toggleBadgeDisplay(userId, badgeId, isDisplayed);
