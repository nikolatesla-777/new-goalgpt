...
import type {
  ...
  PredictionDeletedEvent,
  PredictionCreateData,
  PredictionUpdateData,
} from './predictionEvents';

export class PredictionOrchestrator extends EventEmitter {
  private static instance: PredictionOrchestrator | null = null;

  static getInstance(): PredictionOrchestrator {
    if (!this.instance) {
   ...
   * 5. Release lock
   */
  async createPrediction(data: PredictionCreateData): Promise<{
    ...
    try {
      ...
      if (redisAvailable) {
        lockAcquired = await RedisManager.acquireLock(lockKey, 'ingest', lockTTL);
        if (!lockAcquired) {
          logger.warn(`[PredictionOrchestrator] Lock busy for ${data.external_id}`);
          return { status: 'duplicate', reason: 'Already processing' };
        }
      } else {
        logger.warn('[PredictionOrchestrator] Redis unavailable - proceeding without lock');
      }

      // Step 2: Check duplicate
      const existing = await pool.query(
        'SELECT id FROM ai_predictions WHERE external_id = $1',
        [data.external_id]
      );
      ...
      if (existing.rows.length > 0) {
        ...
        logger.debug(`[PredictionOrchestrator] Duplicate prediction: ${data.external_id}`);
      ...
      this.emit('prediction:created', {
        ...
        timestamp: Date.now(),
      } as PredictionCreatedEvent);

      logger.info(`[PredictionOrchestrator] Created prediction: ${prediction.id} (${data.external_id})`);
      ...
      logger.error('[PredictionOrchestrator] createPrediction error:', error);
  ...
  async updatePrediction(
    ...
    try {
      ...
      if (redisAvailable) {
        lockAcquired = await RedisManager.acquireLock(lockKey, 'update', lockTTL);
        if (!lockAcquired) {
          logger.warn(`[PredictionOrchestrator] Lock busy for prediction ${predictionId}`);
      ...
      logger.info(`[PredictionOrchestrator] Updated prediction: ${predictionId} (fields: ${Object.keys(updates).join(', ')})`);
      ...
      logger.error('[PredictionOrchestrator] updatePrediction error:', error);
   ...
   * 4. Release lock
   */
  async deletePrediction(predictionId: string): Promise<{
    status: 'success' | 'not_found' | 'error';
  }> {
    ...
    try {
      ...
      if (redisAvailable) {
        lockAcquired = await RedisManager.acquireLock(lockKey, 'delete', lockTTL);
        if (!lockAcquired) {
          logger.warn(`[PredictionOrchestrator] Lock busy for prediction ${predictionId}`);
          return { status: 'error' };
        }
      ...
      logger.info(`[PredictionOrchestrator] Deleted prediction: ${predictionId}`);
      ...
      logger.error('[PredictionOrchestrator] deletePrediction error:', error);
