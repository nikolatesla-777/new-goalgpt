...
export class MetadataEnricher {
    private static instance: MetadataEnricher | null = null;

    // Cache competition -> current season mapping (in memory)
    private competitionSeasonCache = new Map<string, string>();
     ...
     */
    static getInstance(): MetadataEnricher {
        if (!this.instance) {
    ...
    async enrichMatch(matchId: string): Promise<EnrichmentResult> {
      ...
      `, [matchId]);
            ...
            if (matchResult.rows.length === 0) {
                logger.warn(`[MetadataEnricher] Match ${matchId} not found`);
          ...
          `, [seasonId, matchId]);
                    ...
                    logger.info(`[MetadataEnricher] ✅ Enriched ${matchId} with season_id ${seasonId}`);
        ...
        `, [analysisSeasonId, matchId]);
                ...
                this.stats.successfulEnrichments++;
                logger.info(`[MetadataEnricher] ✅ Enriched ${matchId} with season_id ${analysisSeasonId} from match/analysis`);
            ...
            logger.debug(`[MetadataEnricher] Could not find season_id for ${matchId}`);
            return { enriched: false };

        } catch (error) {
            this.stats.failedEnrichments++;
            logger.error(`[MetadataEnricher] Error enriching ${matchId}:`, error);
            return { enriched: false };
        }
    }
     ...
     */
    private async findSeasonForCompetition(competitionId: string): Promise<string | null> {
        ...
        // Check Redis cache
        const redisCached = await RedisManager.get(`meta:comp_season:${competitionId}`);
        if (redisCached) {
        ...
        try {
            ...
            const response = await theSportsAPI.get<any>('/competition/info', {
                id: competitionId,
            });
            ...
            if (seasons.length > 0) {
                ...
                if (currentSeason?.id) {
                    ...
                    await RedisManager.set(`meta:comp_season:${competitionId}`, currentSeason.id, 86400);

                    logger.debug(`[MetadataEnricher] Found season ${currentSeason.id} for competition ${competitionId}`);
            ...
            logger.warn(`[MetadataEnricher] Failed to get competition info for ${competitionId}:`, error);
            return null;
        }
    }
     ...
     */
    private async findSeasonFromMatchAnalysis(matchId: string): Promise<string | null> {
        try {
            const response = await theSportsAPI.get<any>('/match/analysis', {
                match_id: matchId,
            });
            ...
            logger.debug(`[MetadataEnricher] match/analysis failed for ${matchId}`);
            return null;
        }
    }
     ...
     */
    async enrichBatch(limit: number = 100): Promise<{ enriched: number; failed: number }> {
        let enriched = 0;
        let failed = 0;
      ...
      `, [limit]);
            ...
            logger.info(`[MetadataEnricher] Batch complete: ${enriched} enriched, ${failed} failed`);

        } catch (error) {
            logger.error('[MetadataEnricher] Batch enrichment error:', error);
        }
...
export const getMetadataEnricher = () => MetadataEnricher.getInstance();
