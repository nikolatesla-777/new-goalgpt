...
export async function getMatchDetailLive(matchId: string): Promise<DetailLiveMatch | null> {
  try {
    ...
    const response = await theSportsAPI.get<DetailLiveResponse>('/match/detail_live', {
      match_id: matchId,
    });
    ...
    logger.error(`[AIPredictionsLiveData] Error fetching match ${matchId}:`, error);
    return null;
  }
}
...
export async function getMatchesDetailLive(matchIds: string[]): Promise<Map<string, DetailLiveMatch>> {
  ...
  try {
    ...
    // Fetch uncached matches in parallel (TheSports API allows batch)
    // Join match IDs with comma for batch request
    ...
    logger.error(`[AIPredictionsLiveData] Error fetching batch matches:`, error);
    return result;
  }
}
