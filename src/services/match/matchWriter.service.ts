...
export type MatchUpdateSource = 'mqtt' | 'api' | 'watchdog' | 'manual';
...
export interface MatchUpdate {
  ...
  awayScore?: number;
  minute?: number | null;
  homeScores?: number[];  // TheSports 7-element array
  awayScores?: number[];
  htHomeScore?: number;
  htAwayScore?: number;
  ended?: boolean;
...
let onMatchUpdated: ((match: any, source: MatchUpdateSource) => void) | null = null;

export function setOnMatchUpdated(callback: (match: any, source: MatchUpdateSource) => void): void {
...
class MatchWriterService {
   ...
   */
  async write(update: MatchUpdate, source: MatchUpdateSource): Promise<WriteResult> {
    ...
    try {
      ...
      if (result.rowCount === 0) {
        ...
        const existsCheck = await pool.query(
          'SELECT external_id, last_updated_at, last_updated_by FROM ts_matches WHERE external_id = $1',
          [update.matchId]
        );
        ...
        this.skipCount++;
        logger.debug(`[MatchWriter] Skipped stale update for ${update.matchId} from ${source}`);
      ...
      if (onMatchUpdated) {
        try {
          ...
          logger.warn(`[MatchWriter] Broadcast failed for ${update.matchId}: ${err.message}`);
        }
      }
      ...
      const duration = Date.now() - startTime;
      logger.error(`[MatchWriter] Error writing ${update.matchId} from ${source}: ${error.message} [${duration}ms]`);
   ...
   */
  async writeBatch(updates: MatchUpdate[], source: MatchUpdateSource): Promise<{ success: number; failed: number; skipped: number }> {
    ...
    logger.info(`[MatchWriter] Batch write complete: ${success} updated, ${skipped} skipped, ${failed} failed`);
   ...
   */
  async forceWrite(update: MatchUpdate, source: 'watchdog' | 'manual'): Promise<WriteResult> {
   ...
   */
  private triggerSettlementIfNeeded(update: MatchUpdate, match: any): void {
    ...
    if (update.statusId === 3) {
      predictionSettlementService.processEvent({
        ...
        minute: 45,
        timestamp: Date.now(),
      }).catch(err => logger.warn(`[MatchWriter] Settlement error: ${err.message}`));
    }
   ...
   */
  getStats(): { writeCount: number; skipCount: number } {
    return { writeCount: this.writeCount, skipCount: this.skipCount };
  }
}

// Singleton export
export const matchWriterService = new MatchWriterService();
