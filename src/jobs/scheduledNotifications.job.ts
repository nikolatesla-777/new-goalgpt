...
export async function runScheduledNotifications() {
  ...
  try {
    ...
    if (pendingNotifications.length === 0) {
      ...
      logger.info(`Found ${pendingNotifications.length} notification(s) to send`);
    }
    ...
    for (const notification of pendingNotifications) {
      try {
        ...
        if (
          ...
          result = await sendPushToAudience(
            notification.target_audience as 'all' | 'vip' | 'free',
            {
              title,
              body,
        ...
        logger.info(
          `Notification sent: ${notification.id} (${result.totalDelivered}/${result.totalSent + result.totalFailed} delivered)`
        );
        ...
        logger.error(`Error sending notification ${notification.id}:`, notifError);
      }
    }
    ...
    if (processedCount > 0) {
      logger.info(`${jobName}: Processed ${processedCount} notification(s) in ${duration}ms`);
    }
  } catch (error: any) {
    // Log job failure
    const duration = Date.now() - startTime;
