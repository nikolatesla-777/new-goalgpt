...
import { DailyPreSyncService } from '../services/thesports/sync/dailyPreSync.service';
 ...
 */
export async function runH2HPreSync(): Promise<void> {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      logger.info('[H2HPreSync] No matches need H2H sync');
      return;
    }

    logger.info(`[H2HPreSync] Found ${result.rows.length} matches without H2H data`);
    ...
    for (const row of result.rows) {
      try {
        ...
        if (success) {
          synced++;
          logger.info(`[H2HPreSync] Synced H2H for match ${row.external_id}`);
        } else {
          logger.debug(`[H2HPreSync] No H2H data available for match ${row.external_id}`);
        }
        ...
        failed++;
        logger.warn(`[H2HPreSync] Failed to sync ${row.external_id}: ${err.message}`);
      }
    }

    logger.info(`[H2HPreSync] Completed: ${synced} synced, ${failed} failed out of ${result.rows.length} matches`);
  } catch (error: any) {
    logger.error(`[H2HPreSync] Job error: ${error.message}`, error);
    throw error;
  } finally {
    client.release();
  }
