...
export type SyncMethod = 'full' | 'incremental' | 'realtime' | 'static';
export type FrequencyUnit = 'second' | 'minute' | 'hour' | 'day';

export interface ApiEndpointConfig {
  /** Full API URL path (relative to base URL) */
  url: string;
  ...
  /** Sync method: full (complete data fetch), incremental (only changes), realtime (live updates), static (rarely changes) */
  ...
  /** Whether this endpoint supports time-based incremental updates */
  supportsTimeIncrement?: boolean;
  
  /** Request rate limit (requests per minute) */
  rateLimit?: number;
  
  /** Request time limit (days before/after today) */
  timeLimit?: {
    before?: number; // days before today
    after?: number;  // days after today
  };
}
 ...
 */
export const THESPORTS_BASE_URL = 'https://api.thesports.com/v1/football';
 ...
 */
export const BASIC_INFO_ENDPOINTS: Record<string, ApiEndpointConfig> = {
  // 1. Category List
  category: {
    url: '/category/list',
 ...
 */
export const BASIC_DATA_ENDPOINTS: Record<string, ApiEndpointConfig> = {
  // 1. Match Recent List
  matchRecent: {
...
export const API_ENDPOINTS = {
  ...
  ...BASIC_DATA_ENDPOINTS,
} as const;

/**
 * Get full URL for an endpoint
 */
export function getEndpointUrl(endpointKey: keyof typeof API_ENDPOINTS): string {
  const endpoint = API_ENDPOINTS[endpointKey];
  if (!endpoint) {
    throw new Error(`Endpoint "${endpointKey}" not found in API_ENDPOINTS`);
  }
  return `${THESPORTS_BASE_URL}${endpoint.url}`;
}

/**
 * Get endpoint configuration by key
 */
export function getEndpointConfig(endpointKey: keyof typeof API_ENDPOINTS): ApiEndpointConfig {
  const endpoint = API_ENDPOINTS[endpointKey];
  if (!endpoint) {
    throw new Error(`Endpoint "${endpointKey}" not found in API_ENDPOINTS`);
  }
 ...
 */
export function getFrequencyInMs(endpointKey: keyof typeof API_ENDPOINTS): number {
  const endpoint = API_ENDPOINTS[endpointKey];
  if (!endpoint) {
    throw new Error(`Endpoint "${endpointKey}" not found in API_ENDPOINTS`);
  }

  const { value, unit } = endpoint.frequency;
...
export type BasicInfoEndpointKey = keyof typeof BASIC_INFO_ENDPOINTS;
export type BasicDataEndpointKey = keyof typeof BASIC_DATA_ENDPOINTS;
