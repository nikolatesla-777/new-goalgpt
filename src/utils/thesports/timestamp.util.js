 ...
 */
var TSI_OFFSET_MS = 3 * 60 * 60 * 1000;
/**
...
function formatTheSportsDate(date) {
    ...
    var tsiMs = date.getTime() + TSI_OFFSET_MS;
    var tsiDate = new Date(tsiMs);
    var year = tsiDate.getUTCFullYear();
    var month = String(tsiDate.getUTCMonth() + 1).padStart(2, '0');
    var day = String(tsiDate.getUTCDate()).padStart(2, '0');
...
function getTodayTSI() {
    var tsiMs = Date.now() + TSI_OFFSET_MS;
    var tsiDate = new Date(tsiMs);
    var year = tsiDate.getUTCFullYear();
    var month = String(tsiDate.getUTCMonth() + 1).padStart(2, '0');
    var day = String(tsiDate.getUTCDate()).padStart(2, '0');
...
function isToday(date) {
    var nowTsiMs = Date.now() + TSI_OFFSET_MS;
    var todayTsi = new Date(nowTsiMs);
    var dateTsiMs = date.getTime() + TSI_OFFSET_MS;
    var dateTsi = new Date(dateTsiMs);
