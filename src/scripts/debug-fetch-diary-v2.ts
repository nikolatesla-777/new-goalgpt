...
async function main() {
    ...
    const dateStr = formatTheSportsDate(new Date()).replace(/-/g, '');
    ...
    try {
        ...
        // Search for Taawon, Saudi, Ittihad in matches AND results_extra
        let found = 0;
        ...
        if (found === 0) {
            console.log('No matches found with Ittihad, Taawon, or Saudi.');
        }
