...
async function main() {
    ...
    try {
        console.log('Searching for "Benfica" or "Estoril" match (Last 24h)...');
        const matchQuery = `
            ...
            WHERE (t1.name ILIKE '%Benfica%' OR t2.name ILIKE '%Benfica%' OR 
                   t1.name ILIKE '%Estoril%' OR t2.name ILIKE '%Estoril%')
        ...
        if (matches.rows.length > 0) {
            // Check predictions for ALL found matches
            for (const match of matches.rows) {
                console.log(`\nChecking Predictions for match ${match.external_id} (${match.home} vs ${match.away})...`);
                const predQuery = `
                ...
                `;
                const preds = await client.query(predQuery, [match.external_id]);
