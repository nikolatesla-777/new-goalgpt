...
export async function createTsVenuesTable(): Promise<void> {
  ...
  try {
    ...
    logger.info('✅ ts_venues table created successfully');
  } catch (error: any) {
    await client.query('ROLLBACK');
    logger.error('❌ Failed to create ts_venues table:', error);
    throw error;
  } finally {
    client.release();
  }
