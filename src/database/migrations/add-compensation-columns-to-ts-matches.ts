...
export async function addCompensationColumnsToTsMatches(): Promise<void> {
  ...
  try {
    ...
    logger.info('✅ Compensation columns added to ts_matches table successfully');
  } catch (error: any) {
    await client.query('ROLLBACK');
    logger.error('❌ Failed to add compensation columns to ts_matches table:', error);
    throw error;
  } finally {
    client.release();
  }
