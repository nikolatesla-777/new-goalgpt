...
import { pool, safeQuery } from '../connection';
import { logger } from '../../utils/logger';

async function createFootyStatsTables(): Promise<void> {
  logger.info('[Migration] Creating FootyStats integration tables...');
  ...
  try {
    await safeQuery(createMappingsTable);
    logger.info('[Migration] ✓ integration_mappings table created');

    await safeQuery(createMatchStatsTable);
    logger.info('[Migration] ✓ fs_match_stats table created');

    await safeQuery(createTeamFormTable);
    logger.info('[Migration] ✓ fs_team_form table created');

    await safeQuery(createRefereesTable);
    logger.info('[Migration] ✓ fs_referees table created');

    await safeQuery(createLeagueStatsTable);
    logger.info('[Migration] ✓ fs_league_stats table created');

    logger.info('[Migration] All FootyStats tables created successfully!');
  } catch (error) {
    logger.error('[Migration] Error creating FootyStats tables:', error);
    throw error;
  }
}
