 ...
 */
export function removeDuplicateIds(records: any[]): { unique: any[]; seenIds: Set<string> } {
 ...
 */
export function removeDuplicateEmails(records: any[]): { unique: any[]; seenEmails: Set<string> } {
 ...
 */
export function filterValidForeignKeys(
  records: any[],
  fkColumn: string,
  validIds: Set<string>
): any[] {
  return records.filter(record => {
    ...
    if (!fkValue || fkValue === '') return false;
 ...
 */
export function getForeignKeyColumn(tableName: string, record: any): string | null {
  if (
    (tableName.includes('customer_') || tableName.includes('support_') || tableName.includes('favorite_')) &&
    tableName !== 'customer_users'
  ) {
    const fkColumn = Object.keys(record).find(
      col => col === 'customer_user_id' || col === 'user_id'
    );
