 ...
 */
function extractChangedMatchIds(payload: any): string[] {
 ...
 */
async function updateMatchDirect(
  matchId: string,
  updates: FieldUpdate[]
  ...
  try {
    ...
    if (statusUpdate && statusUpdate.value !== 8) {
      ...
      if (existing?.status_id === 8) {
        logger.warn(
          `[StateUpdate] REJECT: Match ${matchId} already finished (status=8 immutable)`
        );
    ...
    logger.error(`[StateUpdate] Failed to update ${matchId}:`, error.message);
    return { success: false, fieldsUpdated: [] };
  }
}
 ...
 */
export async function updateLiveMatchStates(
  ...
  try {
    ...
    logger.info(`[StateUpdate] Processing ${changedMatchIds.length} changed matches`);
    ...
    for (const matchId of changedMatchIds) {
      try {
        ...
        if (existsResult.rows.length === 0) {
          logger.debug(`[StateUpdate] Match ${matchId} not in DB, skipping`);
          continue;
        }
        ...
        const matchData = results_list.find(
          (m: any) => String(m?.id || m?.match_id) === String(matchId)
        );

        if (!matchData) {
          logger.debug(`[StateUpdate] Match ${matchId} not in detail_live response`);
          continue;
        }
        ...
        if (newMinute !== null) {
          ...
          const minuteText = generateMinuteText(newMinute, newStatusId);
          updates.push({
            field: 'minute_text',
            value: minuteText,
            source: 'api_state_update',
        ...
        if (result.success) {
          ...
          if (oldStatusId !== newStatusId) {
            ...
            logger.info(
              `[StateUpdate] ✅ Match ${matchId} ${transitionName} (${homeScore}-${awayScore})`
            );
        ...
        logger.error(`[StateUpdate] Error processing ${matchId}:`, matchError.message);
        results.push({
          matchId,
          status: 'error',
    ...
    logger.info(
      `[StateUpdate] Completed in ${duration}ms: ${updatedCount}/${changedMatchIds.length} matches updated`
    );
    ...
    logger.error('[StateUpdate] Error:', error);
