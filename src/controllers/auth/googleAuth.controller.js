 ...
 */
function googleSignIn(request, reply) {
    return __awaiter(this, void 0, void 0, function () {
        var _a, idToken, referralCode, deviceInfo, decodedToken, firebaseAuth, err_1, googleId, email, name_1, picture, user, isNewUser, userId, refError_1, userProfile, tokens, error_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 20, , 21]);
                    _a = request.body, idToken = _a.idToken, referralCode = _a.referralCode, deviceInfo = _a.deviceInfo;
                    if (!idToken) {
                ...
                case 3:
                    err_1 = _b.sent();
                    logger_1.logger.error('Firebase token verification failed:', err_1);
                    return [2 /*return*/, reply.status(401).send({
                            error: 'INVALID_TOKEN',
                            message: 'Invalid or expired Google ID token',
                        })];
                case 4:
                    googleId = decodedToken.sub, email = decodedToken.email, name_1 = decodedToken.name, picture = decodedToken.picture;
                    if (!email) {
                        return [2 /*return*/, reply.status(400).send({
                    ...
                    return [4 /*yield*/, (0, user_repository_1.getUserByOAuthProvider)('google', googleId)];
                case 5:
                    user = _b.sent();
                ...
                case 15:
                    // Update user last login
                    _b.sent();
                    logger_1.logger.info('Existing user logged in via Google OAuth:', { userId: userId, email: email });
                    _b.label = 16;
                case 16:
                ...
                case 20:
                    error_1 = _b.sent();
                    logger_1.logger.error('Google OAuth error:', error_1);
                    return [2 /*return*/, reply.status(500).send({
