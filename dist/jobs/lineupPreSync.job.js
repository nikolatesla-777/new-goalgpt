 ...
 */
async function fetchLineupWithRetry(matchId, retries = 0) {
    try {
        const response = await TheSportsAPIManager_1.theSportsAPI.get('/match/lineup/detail', { match_id: matchId });
        const lineup = parseLineupData(response);
        if (!lineup || (lineup.home.length === 0 && lineup.away.length === 0)) {
            logger_1.logger.debug(`[LineupPreSync] No lineup data available for ${matchId}`);
            return false;
        }
        // Save to database
    ...
    `, [
        ...
        logger_1.logger.info(`[LineupPreSync] ✓ Synced lineup for ${matchId} (${lineup.home.length}H + ${lineup.away.length}A players)`);
        return true;
    }
    catch (error) {
        if (retries < MAX_RETRIES) {
            logger_1.logger.warn(`[LineupPreSync] Retry ${retries + 1}/${MAX_RETRIES} for ${matchId}: ${error.message}`);
            await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
            return fetchLineupWithRetry(matchId, retries + 1);
        }
        logger_1.logger.error(`[LineupPreSync] Failed to fetch lineup for ${matchId} after ${MAX_RETRIES} retries: ${error.message}`);
        return false;
    }
}
/**
 ...
 */
async function getMatchesNeedingLineup() {
 ...
 * Main sync function
 */
async function runLineupPreSync() {
    if (isRunning) {
    ...
    try {
        ...
        logger_1.logger.info(`[LineupPreSync] Found ${matches.length} matches starting in next ${MINUTES_BEFORE_MATCH} minutes without lineup`);
        let successCount = 0;
        let failCount = 0;
        for (const match of matches) {
            const minutesUntilStart = Math.round((match.match_time - Date.now() / 1000) / 60);
            logger_1.logger.debug(`[LineupPreSync] Processing ${match.external_id} (starts in ${minutesUntilStart} min)`);
        ...
        logger_1.logger.info(`[LineupPreSync] Completed: ${successCount} synced, ${failCount} failed (${duration}ms)`);
        if (successCount > 0) {
        ...
        logger_1.logger.error('[LineupPreSync] Job failed:', error.message);
        (0, obsLogger_1.logEvent)('error', 'lineup_presync.failed', { error: error.message });
    }
    finally {
        isRunning = false;
    }
