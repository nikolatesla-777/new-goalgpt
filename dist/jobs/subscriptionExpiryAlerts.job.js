...
async function runSubscriptionExpiryAlerts() {
    ...
    try {
        ...
        const expiringSubscriptions = await kysely_1.db
            ...
            'cs.expired_at',
            'cs.platform',
            (0, kysely_2.sql) `EXTRACT(DAY FROM (cs.expired_at - NOW()))`.as('days_remaining'),
        ])
            ...
            .execute();
        logger_1.logger.info(`Found ${expiringSubscriptions.length} subscription(s) expiring in 3 days`);
        // Send alerts
        for (const subscription of expiringSubscriptions) {
            try {
                const daysRemaining = Math.ceil(Number(subscription.days_remaining));
                await (0, push_service_1.sendPushToUser)(subscription.user_id, {
                ...
                logger_1.logger.error(`Error sending expiry alert to user ${subscription.user_id}:`, userError);
            }
        }
        // Log job success
        ...
        logger_1.logger.info(`${jobName}: Sent ${processedCount} alert(s) in ${duration}ms`);
    }
    catch (error) {
        // Log job failure
        const duration = Date.now() - startTime;
