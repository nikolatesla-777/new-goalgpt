...
exports.coldStartKickoffBackfill = coldStartKickoffBackfill;
 ...
 */
async function coldStartKickoffBackfill() {
    console.log('=====================');
    console.log('[ColdStart] SMART BACKFILL - Starting');
    console.log('[ColdStart] Timestamp:', new Date().toISOString());
    console.log('=====================');
    try {
        logger_1.logger.info('[ColdStart] Starting smart kickoff timestamp backfill...');
        ...
        const result1 = await connection_1.pool.query(`
      UPDATE ts_matches
    ...
    `, [nowTs]);
        ...
        if (result1.rowCount > 0) {
            ...
            logger_1.logger.info(`[ColdStart] ✅ Backfilled first_half_kickoff_ts for ${result1.rowCount} matches (smart: ${smart}, fallback: ${fallback})`);
        ...
        const result2 = await connection_1.pool.query(`
      UPDATE ts_matches
    ...
    `, [nowTs]);
        ...
        if (result2.rowCount > 0) {
            ...
            logger_1.logger.info(`[ColdStart] ✅ Backfilled second_half_kickoff_ts for ${result2.rowCount} matches (smart: ${smart}, fallback: ${fallback})`);
        ...
        if (result1.rowCount === 0 && result2.rowCount === 0) {
            ...
            logger_1.logger.info('[ColdStart] No missing kickoff timestamps found. All good!');
        }
        console.log('[ColdStart] Smart kickoff backfill complete');
        logger_1.logger.info('[ColdStart] Smart kickoff backfill complete');
    }
    catch (error) {
        console.error('[ColdStart] ERROR:', error);
        logger_1.logger.error('[ColdStart] Failed to backfill kickoff timestamps:', error);
