...
exports.MatchFreezeDetectionWorker = void 0;
const matchFreezeDetection_service_1 = require("../services/thesports/match/matchFreezeDetection.service");
const circuitBreaker_1 = require("../utils/circuitBreaker");
const logger_1 = require("../utils/logger");
const obsLogger_1 = require("../utils/obsLogger");
class MatchFreezeDetectionWorker {
    constructor(matchDetailLiveService) {
        this.intervalId = null;
        this.isRunning = false;
        // Cooldown tracking: matchId -> last reconcile attempt timestamp (seconds)
        this.reconcileCooldown = new Map();
        ...
        this.autoEndThresholdSeconds = 7200;
        // Deduplication: matches processed in current window
        this.processedInWindow = new Set();
        this.freezeDetectionService = new matchFreezeDetection_service_1.MatchFreezeDetectionService();
    ...
    async tick() {
        if (this.isRunning) {
            logger_1.logger.debug('[FreezeDetection] Tick already running, skipping this run');
            return;
        }
        this.isRunning = true;
        ...
        try {
            ...
            const frozen = await this.freezeDetectionService.detectFrozenMatches(nowTs, 120, // LIVE stale: 120s
            900, // HALF_TIME stuck: 900s (15 min)
            180 // SECOND_HALF no progress: 180s (3 min)
            );
            if (frozen.length === 0) {
                (0, obsLogger_1.logEvent)('debug', 'freeze.tick', {
                    scanned_count: 0,
                    frozen_count: 0,
            ...
            (0, obsLogger_1.logEvent)('info', 'freeze.tick', {
            ...
            for (const match of frozen) {
                ...
                (0, obsLogger_1.logEvent)('warn', 'match.stale.detected', {
                ...
                if (!cooldownExpired) {
                    ...
                    (0, obsLogger_1.logEvent)('debug', 'match.stale.reconcile.skipped', {
                        match_id: match.matchId,
                        reason: 'cooldown',
                ...
                try {
                    (0, obsLogger_1.logEvent)('warn', 'match.stale.reconcile.requested', {
                        match_id: match.matchId,
                        reason: match.reason,
                    ...
                    (0, obsLogger_1.logEvent)('info', 'match.stale.reconcile.done', {
                        match_id: match.matchId,
                    ...
                    if (reconcileResult.updated) {
                        ...
                        if (match.ageSec >= this.autoEndThresholdSeconds) {
                            try {
                                ...
                                if (forceEnded) {
                                    (0, obsLogger_1.logEvent)('warn', 'match.stale.auto_ended', {
                                        match_id: match.matchId,
                                        reason: match.reason,
                                ...
                                (0, obsLogger_1.logEvent)('error', 'match.stale.force_end_failed', {
                                    match_id: match.matchId,
                        ...
                        (0, obsLogger_1.logEvent)('error', 'match.stale.marked', {
                            match_id: match.matchId,
                            reason: match.reason,
                    ...
                    if (error instanceof circuitBreaker_1.CircuitOpenError) {
                        (0, obsLogger_1.logEvent)('warn', 'match.stale.reconcile.skipped', {
                            match_id: match.matchId,
                            reason: 'circuit_open',
                        });
                    ...
                    (0, obsLogger_1.logEvent)('error', 'match.stale.reconcile.failed', {
                        match_id: match.matchId,
                        error: error.message,
                    });
                    reconcileFailedCount++;
            ...
            (0, obsLogger_1.logEvent)('info', 'freeze.tick.done', {
            ...
            (0, obsLogger_1.logEvent)('error', 'freeze.tick.failed', {
                error: error.message,
            });
            logger_1.logger.error('[FreezeDetection] Error in tick:', error);
        }
        finally {
            this.isRunning = false;
        }
    ...
    start() {
        if (this.intervalId) {
            logger_1.logger.warn('Match freeze detection worker already started');
            return;
        }
    ...
    stop() {
        if (this.intervalId) {
            ...
            this.processedInWindow.clear();
            logger_1.logger.info('Match freeze detection worker stopped');
        }
    }
}
exports.MatchFreezeDetectionWorker = MatchFreezeDetectionWorker;
