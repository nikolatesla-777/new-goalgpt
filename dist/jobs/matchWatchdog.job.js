...
exports.MatchWatchdogWorker = void 0;
const matchWatchdog_service_1 = require("../services/thesports/match/matchWatchdog.service");
...
class MatchWatchdogWorker {
    constructor(matchDetailLiveService, matchRecentService) {
        ...
        this.matchWatchdogService = new matchWatchdog_service_1.MatchWatchdogService();
    ...
    async reconcileViaOrchestrator(matchId) {
        try {
            ...
            if (!matchData) {
                logger_1.logger.info(`[Watchdog.orchestrator] Match ${matchId} not in detail_live, checking /match/detail fallback...`);
                try {
                    ...
                    if (detailMatch && (String(detailMatch.id) === String(matchId) || String(detailMatch.match_id) === String(matchId))) {
                        ...
                        logger_1.logger.info(`[Watchdog.orchestrator] Found match ${matchId} in /match/detail fallback. Status: ${matchData.status_id}`);
                    }
                }
                catch (fallbackError) {
                    // Suppress "Unauthorized" or simple 403 errors to avoid noise
                    logger_1.logger.warn(`[Watchdog.orchestrator] Fallback /match/detail failed for ${matchId}: ${fallbackError.message}`);
                }
            }
            if (!matchData) {
                logger_1.logger.warn(`[Watchdog.orchestrator] Match ${matchId} not found in detail_live OR detail fallback`);
                return {};
            }
            // Step 3: Parse fields
            const updates = [];
            ...
            if (updates.length > 0) {
                ...
                if (result.status === 'success') {
                    (0, obsLogger_1.logEvent)('info', 'watchdog.orchestrator.success', {
                        matchId,
            ...
            logger_1.logger.error(`[Watchdog.orchestrator] Error for match ${matchId}:`, error);
            throw error;
        }
    }
    /**
    ...
    async runGlobalSweep() {
        try {
            ...
            if (count > 0) {
                logger_1.logger.debug(`[Watchdog] Global Sweep: Processing ${count} active live matches.`);
            }
            ...
            for (let i = 0; i < count; i += chunk) {
                ...
                await Promise.all(batch.map(async (m) => {
                    ...
                    try {
                        ...
                        logger_1.logger.warn(`[Watchdog] Global Sweep failed for ${matchId}: ${err.message}`);
                    }
                }));
            ...
            logger_1.logger.error('[Watchdog] Global Sweep Fatal Error:', error);
        }
    }
    /**
    ...
    async tick() {
        if (this.isRunning) {
            logger_1.logger.warn('[Watchdog] Tick already running, skipping this run');
            return;
        }
        this.isRunning = true;
        ...
        try {
            ...
            logger_1.logger.error('[Watchdog] Error stack:', error.stack);
        }
        finally {
            this.isRunning = false;
            logger_1.logger.info('[Watchdog] ========== TICK END ==========');
        }
    }
    /**
     * Start the worker
    ...
    start() {
        if (this.intervalId) {
            logger_1.logger.warn('Match watchdog worker already started');
            return;
        }
        logger_1.logger.info('[Watchdog] Starting MatchWatchdogWorker for should-be-live and stale match detection');
        // Run immediately on start with timeout safety
        logger_1.logger.info('[Watchdog] Calling first tick()...');
        this.tick().catch(err => {
            logger_1.logger.error('[Watchdog] Initial tick() failed:', err);
        ...
        this.intervalId = setInterval(() => {
            this.tick().catch(err => {
                logger_1.logger.error('[Watchdog] Interval tick() failed:', err);
    ...
    stop() {
        if (this.intervalId) {
            ...
            this.intervalId = null;
            logger_1.logger.info('Match watchdog worker stopped');
        }
    }
}
exports.MatchWatchdogWorker = MatchWatchdogWorker;
