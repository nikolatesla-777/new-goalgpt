...
async function getUserProfile(userId) {
    try {
        const userProfile = await kysely_1.db
            ...
            .leftJoin('customer_subscriptions as sub', (join) => join
            .onRef('sub.customer_user_id', '=', 'cu.id')
            .on('sub.status', '=', 'active')
            .on('sub.expired_at', '>', (0, kysely_2.sql) `NOW()`))
            .select([
            'cu.id',
            'cu.email',
            ...
            (0, kysely_2.sql) `CASE WHEN sub.id IS NOT NULL THEN true ELSE false END`.as('is_vip'),
        ])
            .where('cu.id', '=', userId)
            .executeTakeFirst();
        ...
        logger_1.logger.error('[UserRepository] Failed to get user profile:', error);
        throw error;
    }
}
// ============================================================
 ...
 */
async function deactivatePushTokens(userId, deviceId) {
    try {
        const query = kysely_1.db
            .updateTable('customer_push_tokens')
            .set({ is_active: false, updated_at: (0, kysely_2.sql) `NOW()` })
        ...
        logger_1.logger.error('[UserRepository] Failed to deactivate push tokens:', error);
        throw error;
    }
}
/**
 ...
 */
async function upsertPushToken(userId, deviceInfo) {
    try {
        await kysely_1.db
            ...
            .onConflict((oc) => oc.columns(['customer_user_id', 'device_id']).doUpdateSet({
            token: deviceInfo.fcmToken,
            platform: deviceInfo.platform,
            app_version: deviceInfo.appVersion || null,
            is_active: true,
        ...
        logger_1.logger.error('[UserRepository] Failed to upsert push token:', error);
        throw error;
    }
}
// ============================================================
 ...
 */
async function getUserByOAuthProvider(provider, providerId) {
    try {
        ...
        logger_1.logger.error('[UserRepository] Failed to get user by OAuth provider:', error);
        throw error;
    }
}
/**
...
async function getUserByEmail(email) {
    try {
        ...
        logger_1.logger.error('[UserRepository] Failed to get user by email:', error);
        throw error;
    }
}
/**
...
async function getUserByPhone(phone) {
    try {
        ...
        logger_1.logger.error('[UserRepository] Failed to get user by phone:', error);
        throw error;
    }
}
/**
...
async function getUserById(userId) {
    try {
        ...
        logger_1.logger.error('[UserRepository] Failed to get user by ID:', error);
        throw error;
    }
}
/**
...
async function getUserByEmailWithPassword(email) {
    try {
        ...
        logger_1.logger.error('[UserRepository] Failed to get user by email with password:', error);
        throw error;
    }
}
/**
 ...
 * @returns User with password hash or null if not found
 */
async function getUserByPhoneWithPassword(phone) {
    try {
        ...
        logger_1.logger.error('[UserRepository] Failed to get user by phone with password:', error);
        throw error;
    }
}
// ============================================================
 ...
 */
async function createEmailPasswordUser(email, passwordHash, name) {
    try {
        let createdUser;
        const normalizedEmail = email.toLowerCase().trim();
        ...
        logger_1.logger.error('[UserRepository] Failed to create email/password user:', error);
        throw error;
    }
}
/**
 ...
 * @returns Created user's basic info
 */
async function createOAuthUser(oauthData) {
    try {
        let createdUser;
        await kysely_1.db.transaction().execute(async (trx) => {
            // Generate unique referral code
            ...
            await trx
                ...
                is_primary: true,
                last_login_at: (0, kysely_2.sql) `NOW()`,
                linked_at: (0, kysely_2.sql) `NOW()`,
                created_at: (0, kysely_2.sql) `NOW()`,
                updated_at: (0, kysely_2.sql) `NOW()`,
            })
                .execute();
            ...
            await trx
                ...
                .execute();
            logger_1.logger.info(`[UserRepository] New user created via ${oauthData.provider} OAuth:`, {
                userId: newUser.id,
                provider: oauthData.provider,
            });
        });
        ...
        logger_1.logger.error('[UserRepository] Failed to create OAuth user:', error);
        throw error;
    }
}
// ============================================================
 ...
 */
async function getOAuthIdentity(provider, providerId) {
    try {
        const identity = await kysely_1.db
        ...
        logger_1.logger.error('[UserRepository] Failed to get OAuth identity:', error);
        throw error;
    }
}
/**
 ...
 */
async function linkOAuthProvider(userId, oauthData) {
    try {
        await kysely_1.db
            .insertInto('customer_oauth_identities')
            .values({
            ...
            .onConflict((oc) => oc.columns(['customer_user_id', 'provider']).doUpdateSet({
            provider_user_id: oauthData.providerId,
            email: oauthData.email || null,
            display_name: oauthData.name || null,
            profile_photo_url: oauthData.picture || null,
        ...
        logger_1.logger.error('[UserRepository] Failed to link OAuth provider:', error);
        throw error;
    }
}
/**
 ...
 */
async function updateUserInfo(userId, email, name) {
    try {
        const updateData = {};
        if (email)
        ...
        logger_1.logger.error('[UserRepository] Failed to update user info:', error);
        throw error;
    }
}
/**
...
async function updateUserLastLogin(userId) {
    try {
        await kysely_1.db
            .updateTable('customer_users')
            .set({ updated_at: (0, kysely_2.sql) `NOW()` })
            .where('id', '=', userId)
            .execute();
    }
    catch (error) {
        logger_1.logger.error('[UserRepository] Failed to update last login:', error);
        throw error;
    }
}
/**
...
async function getUserAuthContext(userId) {
    try {
        ...
        logger_1.logger.error('[UserRepository] Failed to get user auth context:', error);
        throw error;
    }
}
