...
class MatchStatsRepository {
    ...
    async upsertStats(stats) {
      ...
      `;
            ...
            logger_1.logger.debug(`[MatchStatsRepo] Upserted stats for match ${stats.match_id}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error(`[MatchStatsRepo] Error upserting stats for ${stats.match_id}:`, error.message);
            return false;
        }
        finally {
            client.release();
    ...
    async getStats(matchId) {
        const client = await connection_1.pool.connect();
        try {
            const result = await client.query('SELECT * FROM ts_match_stats WHERE match_id = $1', [matchId]);
            ...
            logger_1.logger.error(`[MatchStatsRepo] Error getting stats for ${matchId}:`, error.message);
            return null;
        }
        finally {
            client.release();
     ...
     */
    parseStatsFromDetailLive(matchData) {
        const stats = {};
