 ...
 *
 * Creates:
 * 1. integration_mappings - TheSports ↔ FootyStats ID mapping (Rosetta Stone)
 * 2. fs_match_stats - Match-level FootyStats data (xG, BTTS, trends)
 * 3. fs_team_form - Team form data from FootyStats
 * 4. fs_referees - Referee statistics
 *
 * @author GoalGPT Team
 * @version 1.0.0
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.createFootyStatsTables = createFootyStatsTables;
const connection_1 = require("../connection");
const logger_1 = require("../../utils/logger");
async function createFootyStatsTables() {
    logger_1.logger.info('[Migration] Creating FootyStats integration tables...');
    // ============================================================================
  ...
  `;
    ...
    CREATE TABLE IF NOT EXISTS fs_match_stats (
      ...
      trends JSONB,
      -- Example: {"home": [["great", "Won last 5 games"]], "away": [["bad", "Lost last 3"]]}

      -- H2H Summary
      h2h_data JSONB,
      -- Example: {"team_a_wins": 3, "team_b_wins": 1, "draws": 2, "btts_pct": 60, "avg_goals": 2.8}
  ...
  `;
    ...
    CREATE TABLE IF NOT EXISTS fs_team_form (
      ...
      goal_timing JSONB,
      -- Example: {"0-15": 12, "16-30": 18, "31-45": 15, "46-60": 20, "61-75": 18, "76-90": 17}

      -- Metadata
      last_x_matches INTEGER DEFAULT 5,  -- 5, 6, or 10
      fetched_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,

      -- Constraints (team + season + last_x is unique)
  ...
  `;
    ...
    try {
        await (0, connection_1.safeQuery)(createMappingsTable);
        logger_1.logger.info('[Migration] ✓ integration_mappings table created');
        await (0, connection_1.safeQuery)(createMatchStatsTable);
        logger_1.logger.info('[Migration] ✓ fs_match_stats table created');
        await (0, connection_1.safeQuery)(createTeamFormTable);
        logger_1.logger.info('[Migration] ✓ fs_team_form table created');
        await (0, connection_1.safeQuery)(createRefereesTable);
        logger_1.logger.info('[Migration] ✓ fs_referees table created');
        await (0, connection_1.safeQuery)(createLeagueStatsTable);
        logger_1.logger.info('[Migration] ✓ fs_league_stats table created');
        logger_1.logger.info('[Migration] All FootyStats tables created successfully!');
    }
    catch (error) {
        logger_1.logger.error('[Migration] Error creating FootyStats tables:', error);
        throw error;
    }
}
// Run migration if executed directly
if (require.main === module) {
    createFootyStatsTables()
        .then(() => {
