...
async function up(db) {
    ...
    const xpResult = await (0, kysely_1.sql) `
  ...
  `.execute(db);
    ...
    const creditsResult = await (0, kysely_1.sql) `
  ...
  `.execute(db);
    ...
    const vipUsers = await db
        ...
        .where('cs.expired_at', '>', (0, kysely_1.sql) `NOW()`)
    ...
    for (const user of vipUsers) {
        await db.transaction().execute(async (trx) => {
            ...
            await trx
                ...
                lifetime_earned: (0, kysely_1.sql) `lifetime_earned + 50`,
            ...
            await trx
                ...
                metadata: (0, kysely_1.sql) `'{"migration": true}'::jsonb`
            })
                .execute();
        });
        bonusCount++;
    ...
    const defaultBadges = [
        {
            ...
            category: 'milestone',
            rarity: 'common',
            unlock_condition: (0, kysely_1.sql) `'{"type": "referrals", "count": 1}'::jsonb`,
            reward_xp: 50,
            reward_credits: 10
        },
        {
            ...
            rarity: 'rare',
            unlock_condition: (0, kysely_1.sql) `'{"type": "predictions", "correct_count": 10}'::jsonb`,
            reward_xp: 200,
            reward_credits: 50
        },
        {
            ...
            category: 'milestone',
            rarity: 'common',
            unlock_condition: (0, kysely_1.sql) `'{"type": "login_streak", "days": 7}'::jsonb`,
            reward_xp: 100,
            reward_credits: 25
        },
        {
            ...
            category: 'milestone',
            rarity: 'common',
            unlock_condition: (0, kysely_1.sql) `'{"type": "comments", "count": 1}'::jsonb`,
            reward_xp: 25,
            reward_credits: 5
        },
        {
            ...
            category: 'special',
            rarity: 'epic',
            unlock_condition: (0, kysely_1.sql) `'{"type": "subscription", "any": true}'::jsonb`,
            reward_xp: 500,
            reward_credits: 100
        }
    ];
  ...
  `.execute(db);
    ...
    // Summary
    const summary = await (0, kysely_1.sql) `
    SELECT
