...
exports.renameOrderToSortOrderInStages = renameOrderToSortOrderInStages;
const connection_1 = require("../connection");
const logger_1 = require("../../utils/logger");
async function renameOrderToSortOrderInStages() {
    ...
    `);
      ...
      `);
            logger_1.logger.info('✅ Renamed column "order" to "sort_order" in ts_stages table');
        }
        else {
            logger_1.logger.info('ℹ️ Column "order" does not exist, skipping rename');
        }
      ...
      `);
            ...
            logger_1.logger.debug('Index idx_ts_stages_season_order does not exist or already dropped');
        }
    ...
    `);
        ...
        logger_1.logger.info('✅ Migration completed: order -> sort_order');
    }
    catch (error) {
        await client.query('ROLLBACK');
        logger_1.logger.error('❌ Failed to rename order column:', error);
        throw error;
    }
    finally {
        client.release();
