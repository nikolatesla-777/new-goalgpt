 ...
 */
async function getLeagueById(request, reply) {
    ...
    `, [league_id]);
        ...
        logger_1.logger.error('Error fetching league:', { message: error.message, stack: error.stack });
 ...
 */
async function getLeagueTeams(request, reply) {
    const { league_id } = request.params;
    const client = await connection_1.pool.connect();
    try {
    ...
    `, [league_id]);
      ...
      `, [league_id]);
        ...
        logger_1.logger.error('Error fetching league teams:', error);
 ...
 */
async function getLeagueFixtures(request, reply) {
    const { league_id } = request.params;
    ...
    `, [...params, parseInt(limit)]);
        ...
        logger_1.logger.error('Error fetching league fixtures:', error);
        return reply.status(500).send({ error: 'Internal server error' });
    }
    finally {
        client.release();
 ...
 */
async function getLeagueStandings(request, reply) {
    const { league_id } = request.params;
    const client = await connection_1.pool.connect();
    try {
    ...
    `, [seasonId]);
        ...
        logger_1.logger.error('Error fetching league standings:', error);
        return reply.status(500).send({ error: 'Internal server error' });
    }
    finally {
        client.release();
