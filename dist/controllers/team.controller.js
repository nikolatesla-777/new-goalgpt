 ...
 */
const getTeamById = async (request, reply) => {
    try {
        const { team_id } = request.params;
        const client = await connection_1.pool.connect();
        try {
            // Get team info from ts_teams
            const teamResult = await client.query(`SELECT external_id, name, short_name, logo_url, country_id, competition_id
         FROM ts_teams WHERE external_id = $1`, [team_id]);
            ...
            const seasonResult = await client.query(`SELECT season_id, competition_id 
         FROM ts_matches 
         ...
         LIMIT 1`, [team_id]);
            ...
            if (team.competition_id) {
                const compResult = await client.query(`SELECT external_id, name, logo_url FROM ts_competitions WHERE external_id = $1`, [team.competition_id]);
                competition = compResult.rows[0] || null;
            }
            // Get recent form (last 5 matches)
            const formResult = await client.query(`SELECT 
          m.external_id,
          m.home_team_id,
          m.away_team_id,
        ...
        logger_1.logger.error('[TeamController] Error in getTeamById:', error);
 ...
 */
const getTeamFixtures = async (request, reply) => {
    try {
        const { team_id } = request.params;
        ...
        try {
         ...
         WHERE ${whereClause}
         ORDER BY m.match_time DESC
         LIMIT $${paramIndex}`, [...params, parseInt(limit)]);
        ...
        logger_1.logger.error('[TeamController] Error in getTeamFixtures:', error);
        reply.status(500).send({
            success: false,
            message: error.message || 'Internal server error',
        });
 ...
 */
const getTeamStandings = async (request, reply) => {
    const client = await connection_1.pool.connect();
    try {
        const { team_id } = request.params;
        let { season_id } = request.query;
        // Get team's competition_id for finding the right standings
        const teamResult = await client.query(`SELECT competition_id FROM ts_teams WHERE external_id = $1`, [team_id]);
        ...
        if (!season_id) {
            const seasonResult = await client.query(`SELECT DISTINCT m.season_id, m.competition_id, COUNT(*) as match_count
         FROM ts_matches m
         WHERE (m.home_team_id = $1 OR m.away_team_id = $1)
         ...
         LIMIT 5`, [team_id]);
            ...
            const preferredSeason = seasonResult.rows.find((r) => r.competition_id === teamCompetitionId);
        ...
        const dbStandings = await client.query(`SELECT standings, competition_id FROM ts_standings WHERE season_id = $1`, [season_id]);
        let standingsData = [];
        let source = 'database';
        ...
        if (standingsData.length === 0) {
            logger_1.logger.info(`[TeamController] No standings in DB for season ${season_id}, trying API...`);
            try {
                ...
                if (apiResponse.results && Array.isArray(apiResponse.results)) {
                    ...
                    const existingRow = await client.query(`SELECT id FROM ts_standings WHERE season_id = $1`, [season_id]);
                    if (existingRow.rows.length === 0) {
                        await client.query(`INSERT INTO ts_standings (id, season_id, standings, updated_at)
               VALUES (gen_random_uuid(), $1, $2, NOW())`, [season_id, JSON.stringify(standingsData)]);
                        logger_1.logger.info(`[TeamController] Saved standings to DB for season ${season_id}`);
                    }
                    else {
                        await client.query(`UPDATE ts_standings SET standings = $1, updated_at = NOW() WHERE season_id = $2`, [JSON.stringify(standingsData), season_id]);
                        logger_1.logger.info(`[TeamController] Updated standings in DB for season ${season_id}`);
                    }
                }
            }
            catch (apiError) {
                logger_1.logger.warn(`[TeamController] API fallback failed: ${apiError.message}`);
            }
        }
        if (standingsData.length === 0) {
            reply.status(404).send({
        ...
        const teamStanding = standingsData.find((s) => s.team_id === team_id);
        if (!teamStanding) {
        ...
        logger_1.logger.error('[TeamController] Error in getTeamStandings:', error);
        reply.status(500).send({
            success: false,
            message: error.message || 'Internal server error',
        });
 ...
 */
const searchTeams = async (request, reply) => {
    try {
        ...
        const client = await connection_1.pool.connect();
        try {
            const result = await client.query(`SELECT external_id, name, logo_url, country_id 
         FROM ts_teams 
         WHERE name ILIKE $1 
         ORDER BY name ASC 
         LIMIT $2`, [`%${q}%`, parseInt(limit)]);
            reply.send({
                success: true,
        ...
        logger_1.logger.error('[TeamController] Error in searchTeams:', error);
        reply.status(500).send({
            success: false,
            message: error.message || 'Internal server error',
        });
