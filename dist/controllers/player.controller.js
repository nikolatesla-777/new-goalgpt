 ...
 */
async function getPlayerById(request, reply) {
    ...
    `, [JSON.stringify([{ player_id: playerId }])]);
        ...
        const matches = matchesResult.rows.map(match => {
            ...
            if (match.player_stats) {
                const stats = Array.isArray(match.player_stats)
                    ? match.player_stats
                    : [];
                const playerMatchStats = stats.find((s) => s.player_id === playerId);
                if (playerMatchStats) {
                    playerStats = {
        ...
        logger_1.logger.error('Error fetching player:', error);
 ...
 */
async function searchPlayers(request, reply) {
    let client;
    try {
    ...
    `, [`%${q}%`, limit]);
        ...
        logger_1.logger.error('Error searching players:', { error: error.message, stack: error.stack });
 ...
 */
async function getPlayersByTeam(request, reply) {
    ...
    `, [teamId]);
        ...
        logger_1.logger.error('Error fetching team players:', error);
