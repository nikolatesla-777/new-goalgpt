...
const mapping_service_1 = require("../services/footystats/mapping.service");
const footystats_client_1 = require("../services/footystats/footystats.client");
const logger_1 = require("../utils/logger");
const auth_middleware_1 = require("../middleware/auth.middleware");
// PR-4: Use repository for all FootyStats DB access
const footystats_repository_1 = require("../repositories/footystats.repository");
async function footyStatsRoutes(fastify) {
    // NOTE: Debug endpoint /footystats/debug-db DELETED for security (exposed DB schema)
    // Search competitions by name or country
    fastify.get('/footystats/search-leagues', async (request, reply) => {
        try {
            ...
            const leagues = await (0, footystats_repository_1.getLeagues)(query, params);
    ...
    fastify.get('/footystats/health', async (request, reply) => {
        const apiHealth = footystats_client_1.footyStatsAPI.getHealth();
        const stats = await mapping_service_1.mappingService.getStats();
        return {
            api: apiHealth,
            mappings: stats,
        };
    ...
    fastify.get('/footystats/test', { preHandler: [auth_middleware_1.requireAuth, auth_middleware_1.requireAdmin] }, async (request, reply) => {
        try {
            const { q } = request.query;
            const response = await footystats_client_1.footyStatsAPI.getLeagueList();
            let leagues = response.data || [];
            // Filter if query provided
            if (q) {
                const searchTerm = q.toLowerCase();
                leagues = leagues.filter(l => l.name?.toLowerCase().includes(searchTerm) ||
    ...
    // Run league mapping
    fastify.post('/footystats/mapping/leagues', async (request, reply) => {
        try {
            logger_1.logger.info('[FootyStats] Starting league mapping via API...');
            const stats = await mapping_service_1.mappingService.mapLeagues();
            return {
                success: true,
                stats,
            };
            ...
            logger_1.logger.error('[FootyStats] League mapping failed:', error);
            return reply.status(500).send({
                success: false,
                error: error.message,
            });
    ...
    fastify.post('/footystats/mapping/teams/:leagueId', async (request, reply) => {
        try {
            const { leagueId } = request.params;
            logger_1.logger.info(`[FootyStats] Starting team mapping for league ${leagueId}...`);
            const stats = await mapping_service_1.mappingService.mapTeamsForLeague(leagueId);
            return {
                success: true,
                stats,
            };
            ...
            logger_1.logger.error('[FootyStats] Team mapping failed:', error.message || error);
            return reply.status(500).send({
                success: false,
    ...
    fastify.post('/footystats/mapping/teams', async (request, reply) => {
        try {
            logger_1.logger.info('[FootyStats] Starting team mapping via API...');
            const stats = await mapping_service_1.mappingService.mapAllTeams();
            return {
                success: true,
                stats,
            };
    ...
    // Get mapping stats
    fastify.get('/footystats/mapping/stats', async (request, reply) => {
        const stats = await mapping_service_1.mappingService.getStats();
        return stats;
    });
    // Get unverified mappings
    fastify.get('/footystats/mapping/unverified', async (request, reply) => {
        const unverified = await mapping_service_1.mappingService.getUnverifiedMappings();
        return {
    ...
    fastify.get('/footystats/mapping/verified-leagues', async (request, reply) => {
        // PR-4: Use repository for DB access
        const verified = await (0, footystats_repository_1.getVerifiedLeagueMappings)();
        return { count: verified.length, leagues: verified };
    });
    // Search mappings by name
    fastify.get('/footystats/mapping/search', async (request, reply) => {
        const { q } = request.query;
        if (!q) {
            return reply.status(400).send({ error: 'q parameter required' });
        }
        // PR-4: Use repository for DB access
        const results = await (0, footystats_repository_1.searchMappings)(q);
        return { count: results.length, mappings: results };
    });
    // Verify a mapping
    fastify.post('/footystats/mapping/verify', async (request, reply) => {
        ...
        await mapping_service_1.mappingService.verifyMapping(entity_type, ts_id, 'api');
        return { success: true };
    });
    // ============================================================================
    ...
    fastify.get('/footystats/analysis/:matchId', async (request, reply) => {
        try {
            const { matchId } = request.params;
            // 1. Get match details from TheSports database
            // Note: external_id is varchar, id is UUID - we use external_id for lookups
            // PR-4: Use repository for DB access
            const matchResult = await (0, footystats_repository_1.getMatchDetails)(matchId);
            ...
            // PR-4: Use repository for DB access
            const homeTeamMapping = await (0, footystats_repository_1.getTeamMapping)(match.home_team_name);
            const awayTeamMapping = await (0, footystats_repository_1.getTeamMapping)(match.away_team_name);
            ...
            try {
                ...
                const todaysMatches = await footystats_client_1.footyStatsAPI.getTodaysMatches();
                if (todaysMatches.data) {
                    // Try to find the match by team names
                    fsMatchData = todaysMatches.data.find((m) => (m.home_name?.toLowerCase().includes(match.home_team_name?.toLowerCase().split(' ')[0]) ||
                ...
                if (homeTeamMapping.length > 0) {
                    const response = await footystats_client_1.footyStatsAPI.getTeamLastX(homeTeamMapping[0].fs_id);
                    fsHomeTeamData = response.data?.[0];
                }
                if (awayTeamMapping.length > 0) {
                    const response = await footystats_client_1.footyStatsAPI.getTeamLastX(awayTeamMapping[0].fs_id);
                    fsAwayTeamData = response.data?.[0];
                }
            }
            catch (apiError) {
                logger_1.logger.warn(`[FootyStats] API error fetching match data: ${apiError.message}`);
            }
            ...
            logger_1.logger.error('[FootyStats] Analysis endpoint error:', error);
            return reply.status(500).send({ error: error.message });
        }
    });
    // Get today's matches with FootyStats data
    fastify.get('/footystats/today', async (request, reply) => {
        try {
            const response = await footystats_client_1.footyStatsAPI.getTodaysMatches();
            if (!response.data || response.data.length === 0) {
                return { count: 0, matches: [] };
            }
            // Return matches with potentials
            const matches = response.data.map((m) => ({
                fs_id: m.id,
            ...
            logger_1.logger.error('[FootyStats] Today matches error:', error);
            return reply.status(500).send({ error: error.message });
        }
    });
    // Get detailed FootyStats match data by fs_id
    fastify.get('/footystats/match/:fsId', async (request, reply) => {
        try {
            ...
            let fsMatch = null;
            try {
                const matchResponse = await footystats_client_1.footyStatsAPI.getMatchDetails(fsIdNum);
                fsMatch = matchResponse.data;
                logger_1.logger.info(`[FootyStats] Got match details for ${fsIdNum}`);
            }
            catch (matchErr) {
                logger_1.logger.warn(`[FootyStats] Could not get match details: ${matchErr.message}`);
                // Fallback to today's matches
                const todaysMatches = await footystats_client_1.footyStatsAPI.getTodaysMatches();
                fsMatch = todaysMatches.data?.find((m) => m.id === fsIdNum);
            }
            if (!fsMatch) {
            ...
            try {
                if (fsMatch.homeID) {
                    const homeResponse = await footystats_client_1.footyStatsAPI.getTeamLastX(fsMatch.homeID);
                    // Get last 5 matches data (first entry)
                    const homeData = homeResponse.data?.find((d) => d.last_x_match_num === 5) || homeResponse.data?.[0];
                    homeTeamStats = homeData?.stats;
                    logger_1.logger.info(`[FootyStats] Got home team stats for ${fsMatch.homeID}: PPG=${homeTeamStats?.seasonPPG_overall || 'no data'}`);
                }
                if (fsMatch.awayID) {
                    const awayResponse = await footystats_client_1.footyStatsAPI.getTeamLastX(fsMatch.awayID);
                    const awayData = awayResponse.data?.find((d) => d.last_x_match_num === 5) || awayResponse.data?.[0];
                    awayTeamStats = awayData?.stats;
                    logger_1.logger.info(`[FootyStats] Got away team stats for ${fsMatch.awayID}: PPG=${awayTeamStats?.seasonPPG_overall || 'no data'}`);
                }
            }
            catch (teamError) {
                logger_1.logger.warn(`[FootyStats] Could not fetch team form: ${teamError.message}`);
            }
            ...
            const response = {
                ...
                trends: {
                    ...
                    away: (fsMatch.trends?.away || []).map((t) => ({
                        sentiment: Array.isArray(t) ? t[0] : (t.sentiment || 'neutral'),
                        text: Array.isArray(t) ? t[1] : (t.text || String(t)),
                    })),
                },
            ...
            logger_1.logger.error('[FootyStats] Match detail error:', error);
            return reply.status(500).send({ error: error.message });
        }
    });
    // Clear all mappings (for re-run) - ADMIN ONLY
    fastify.delete('/footystats/mapping/clear', { preHandler: [auth_middleware_1.requireAuth, auth_middleware_1.requireAdmin] }, async (request, reply) => {
        try {
            // PR-4: Use repository for DB access
            await (0, footystats_repository_1.clearAllMappings)();
    ...
    fastify.post('/footystats/migrate', { preHandler: [auth_middleware_1.requireAuth, auth_middleware_1.requireAdmin] }, async (request, reply) => {
        try {
            // PR-4: Use repository for DB access
            await (0, footystats_repository_1.runMigrations)();
