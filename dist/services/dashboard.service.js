...
async function getDashboardStats(period = 'month') {
    ...
    `;
        const revenueResult = await connection_1.pool.query(revenueQuery, [periodStart, previous]);
      ...
      FROM customer_subscriptions
    `;
        const subscribersResult = await connection_1.pool.query(subscribersQuery, [periodStart]);
      ...
      FROM customer_subscriptions
    `;
        const salesResult = await connection_1.pool.query(salesQuery, [periodStart, previous]);
    ...
    `;
        const errorsResult = await connection_1.pool.query(errorsQuery);
    ...
    `;
        const signupsResult = await connection_1.pool.query(signupsQuery, [periodStart, previous]);
    ...
    `;
        const trialsResult = await connection_1.pool.query(trialsQuery);
    ...
    `;
        const firstPurchaseResult = await connection_1.pool.query(firstPurchaseQuery, [periodStart]);
    ...
    `;
        const cancellationsResult = await connection_1.pool.query(cancellationsQuery, [periodStart, previous]);
      ...
      WHERE deleted_at IS NULL
    `;
        const totalMembersResult = await connection_1.pool.query(totalMembersQuery);
...
async function getRevenueChart(period = 'month') {
    try {
      ...
      ORDER BY date ASC
    `;
        const result = await connection_1.pool.query(query, [interval]);
        ...
        logger_1.logger.error('Error fetching revenue chart:', error);
        return [];
    }
}
async function getTrendData(tableName, valueColumn, dateColumn, period, whereClause = '') {
    ...
    try {
        const query = `
            ...
            ORDER BY date ASC
        `;
        const result = await connection_1.pool.query(query, [periodStart]);
        return result.rows.map(row => ({
        ...
        logger_1.logger.error(`Error fetching trend data for ${tableName}:`, error);
        return [];
    }
}
...
async function getSubscribersTrend(period) {
...
async function getRevenueDetails(period, limit = 100) {
    ...
    try {
        const query = `
            ...
            LIMIT $2
        `;
        const result = await connection_1.pool.query(query, [periodStart, limit]);
        return result.rows.map(row => ({
            id: row.id,
            user_id: row.user_id,
            email: row.email || '',
        ...
        logger_1.logger.error('Error fetching revenue details:', error);
        return [];
    }
}
async function getActiveSubscribersDetails(period, limit = 100) {
    try {
        const query = `
            SELECT 
                cs.id,
        ...
        logger_1.logger.error('Error fetching active subscribers details:', error);
        return [];
    }
}
async function getSignupsDetails(period, limit = 100) {
    const { periodStart } = getPeriodDates(period);
    try {
        const query = `
            SELECT 
        ...
        logger_1.logger.error('Error fetching signups details:', error);
        return [];
    }
}
async function getTrialsDetails(period, limit = 100) {
    try {
        const query = `
            SELECT 
                cs.id,
        ...
        logger_1.logger.error('Error fetching trials details:', error);
        return [];
    }
}
async function getCancellationsDetails(period, limit = 100) {
    const { periodStart } = getPeriodDates(period);
    try {
        const query = `
            SELECT 
        ...
        logger_1.logger.error('Error fetching cancellations details:', error);
        return [];
    }
}
async function getChurnDetails(period, limit = 100) {
    const { periodStart } = getPeriodDates(period);
    try {
        const query = `
            SELECT 
        ...
        logger_1.logger.error('Error fetching churn details:', error);
        return [];
    }
}
async function getBillingErrorsDetails(period, limit = 100) {
    try {
        const query = `
            SELECT 
                cs.id,
        ...
        logger_1.logger.error('Error fetching billing errors details:', error);
        return [];
    }
}
// ============ FIRST PURCHASE ============
async function getFirstPurchaseTrend(period) {
    const { periodStart } = getPeriodDates(period);
    try {
        const query = `
      SELECT 
    ...
    `;
        ...
        logger_1.logger.error('Error fetching first purchase trend:', error);
        return [];
    }
}
async function getFirstPurchaseDetails(period, limit = 100) {
    const { periodStart } = getPeriodDates(period);
    try {
        const query = `
      SELECT 
    ...
    `;
        ...
        logger_1.logger.error('Error fetching first purchase details:', error);
        return [];
    }
}
// ============ CONVERSION RATE ============
async function getConversionTrend(period) {
    const { periodStart } = getPeriodDates(period);
    try {
    ...
    `;
        ...
        logger_1.logger.error('Error fetching conversion trend:', error);
        return [];
    }
}
async function getConversionDetails(period, limit = 100) {
    const { periodStart } = getPeriodDates(period);
    try {
    ...
    `;
        ...
        logger_1.logger.error('Error fetching conversion details:', error);
        return [];
    }
}
// ============ TOTAL MEMBERS ============
async function getTotalMembersTrend(period) {
    const { periodStart } = getPeriodDates(period);
    try {
    ...
    `;
        ...
        const baseResult = await connection_1.pool.query(baseQuery, [periodStart]);
        ...
        logger_1.logger.error('Error fetching total members trend:', error);
        return [];
    }
}
async function getTotalMembersDetails(period, limit = 100) {
    try {
    ...
    `;
        ...
        logger_1.logger.error('Error fetching total members details:', error);
        return [];
    }
}
