...
class UnifiedPredictionService {
    ...
    async getPredictions(filter = {}) {
        const { status = 'all', bot, date, access = 'all', page = 1, limit = 50 } = filter;
    ...
    `;
        try {
            ...
            const stats = {
                ...
                winRate: this.calculateWinRate(parseInt(statsRow?.won || '0'), parseInt(statsRow?.lost || '0'))
            };
            const bots = botStatsResult.rows.map(row => ({
                name: row.name,
                ...
                winRate: this.calculateWinRate(parseInt(row.won || '0'), parseInt(row.lost || '0'))
            }));
            ...
            logger_1.logger.error('[UnifiedPredictionService] getPredictions error:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async getBotDetail(botName, page = 1, limit = 50) {
        return this.getPredictions({ bot: botName, page, limit }).then(result => {
            const bot = result.bots.find(b => b.name.toLowerCase().includes(botName.toLowerCase())) || {
                name: botName,
                displayName: botName,
     ...
     */
    async updatePredictionResult(predictionId, result, finalScore) {
        try {
      ...
      `, [result, finalScore, predictionId]);
            logger_1.logger.info(`[UnifiedPredictionService] Updated prediction ${predictionId} to ${result}`);
        }
        catch (error) {
            logger_1.logger.error('[UnifiedPredictionService] updatePredictionResult error:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async matchPrediction(predictionId, matchId, matchUuid, confidence, homeTeamId, awayTeamId) {
        try {
      ...
      `, [
            ...
            logger_1.logger.info(`[UnifiedPredictionService] Matched prediction ${predictionId} to match ${matchId}`);
        }
        catch (error) {
            logger_1.logger.error('[UnifiedPredictionService] matchPrediction error:', error);
            throw error;
        }
    }
    /**
    ...
    async processMatchEvent(event) {
        ...
        if (type === 'MATCH_STATE_CHANGE' && statusId === 8) {
            logger_1.logger.debug(`[UnifiedPredictionService] Match ${matchId} ended - settlement handled by predictionSettlementService`);
        }
    }
    /**
     * Check prediction win - Phase 2: Uses prediction_threshold column directly
     */
    checkPredictionWin(prediction, threshold, totalGoals) {
