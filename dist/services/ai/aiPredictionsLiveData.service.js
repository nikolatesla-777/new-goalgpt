...
function parseDetailLiveMatch(raw) {
    if (!raw.score || !Array.isArray(raw.score) || raw.score.length < 5) {
        logger_1.logger.warn(`[AIPredictionsLiveData] Invalid score format for match ${raw.id}`);
        return null;
    }
...
const API_TIMEOUT_MS = 5000; // 5 seconds timeout (balance between speed and reliability)
 ...
 */
async function getMatchDetailLive(matchId) {
    try {
        // Check cache first
        ...
        const response = await withTimeout(TheSportsAPIManager_1.theSportsAPI.get('/match/detail_live', {
            match_id: matchId,
        }), API_TIMEOUT_MS);
        ...
        logger_1.logger.warn(`[AIPredictionsLiveData] Failed to fetch match ${matchId}: ${errorMsg}`);
 ...
 */
async function getMatchesDetailLive(matchIds) {
    ...
    try {
        ...
        logger_1.logger.debug(`[AIPredictionsLiveData] Fetching ${uncached.length} matches (${matchIds.length} total, ${result.size} cached)`);
        // CRITICAL FIX: TheSports API returns ALL live matches (ignores match_id param!)
        // Solution: Fetch once and filter locally
        let successCount = 0;
        let failCount = 0;
        try {
            const response = await withTimeout(TheSportsAPIManager_1.theSportsAPI.get('/match/detail_live'), API_TIMEOUT_MS);
            if (response.results && Array.isArray(response.results)) {
                logger_1.logger.debug(`[AIPredictionsLiveData] Received ${response.results.length} live matches from API`);
                ...
                logger_1.logger.debug(`[AIPredictionsLiveData] Successfully parsed ${liveMatchesMap.size} matches`);
            ...
            if (successCount > 0) {
                logger_1.logger.debug(`[AIPredictionsLiveData] Successfully fetched ${successCount}/${uncached.length} matches`);
            }
            if (failCount > 0) {
                logger_1.logger.debug(`[AIPredictionsLiveData] ${failCount}/${uncached.length} matches not found in live feed`);
            }
        }
        catch (error) {
            const errorMsg = error.message || 'Unknown error';
            logger_1.logger.warn(`[AIPredictionsLiveData] Failed to fetch live matches: ${errorMsg}`);
        }
        return result;
    }
    catch (error) {
        const errorMsg = error.message || 'Unknown error';
        logger_1.logger.warn(`[AIPredictionsLiveData] Batch fetch failed (${matchIds.length} matches): ${errorMsg}`);
...
function cleanupExpiredCache() {
    ...
    if (cleaned > 0) {
        logger_1.logger.debug(`[AIPredictionsLiveData] Cleaned ${cleaned} expired cache entries`);
    }
}
/**
...
function enforceCacheSizeLimit() {
    ...
    logger_1.logger.warn(`[AIPredictionsLiveData] Cache size limit reached, removed ${toRemove} oldest entries`);
}
