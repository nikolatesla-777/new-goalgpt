...
const categorySync_job_1 = require("../jobs/categorySync.job");
const countrySync_job_1 = require("../jobs/countrySync.job");
const competitionSync_job_1 = require("../jobs/competitionSync.job");
const teamSync_job_1 = require("../jobs/teamSync.job");
const matchDiary_service_1 = require("./thesports/match/matchDiary.service");
const matchSync_service_1 = require("./thesports/match/matchSync.service");
const teamData_service_1 = require("./thesports/team/teamData.service");
const competition_service_1 = require("./thesports/competition/competition.service");
...
class BootstrapService {
    constructor() {
        ...
        this.categorySyncWorker = new categorySync_job_1.CategorySyncWorker();
        this.countrySyncWorker = new countrySync_job_1.CountrySyncWorker();
        this.competitionSyncWorker = new competitionSync_job_1.CompetitionSyncWorker();
        this.teamSyncWorker = new teamSync_job_1.TeamSyncWorker();
    ...
    async isDbEmpty() {
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.info(`Database status: Categories=${categoryCount}, Competitions=${competitionCount}, Teams=${teamCount}, Empty=${isEmpty}`);
                return isEmpty;
            }
            finally {
                client.release();
            ...
            logger_1.logger.warn('⚠️ Database connection failed during bootstrap check. Skipping bootstrap syncs. Error:', error.message);
     ...
     */
    async init() {
        logger_1.logger.info('🚀 Starting Bootstrap Sequence...');
        try {
            ...
            if (isEmpty) {
                logger_1.logger.info('📦 Database is empty. Running initial syncs...');
                // Step 1.1: Sync Categories (foundation)
                logger_1.logger.info('📋 Syncing Categories...');
                try {
                    await this.categorySyncWorker.syncAllCategories();
                    logger_1.logger.info('✅ Categories synced');
                }
                catch (error) {
                    logger_1.logger.error('Failed to sync categories:', error.message);
                    // Continue - categories might not be critical for matches
                }
                // Step 1.2: Sync Countries (depends on Categories)
                logger_1.logger.info('🌍 Syncing Countries...');
                try {
                    await this.countrySyncWorker.syncAllCountries();
                    logger_1.logger.info('✅ Countries synced');
                }
                catch (error) {
                    logger_1.logger.error('Failed to sync countries:', error.message);
                    // Continue - countries might not be critical for matches
                }
                // Step 1.3: Sync Competitions (depends on Categories/Countries)
                logger_1.logger.info('🏆 Syncing Competitions...');
                await this.competitionSyncWorker.syncCompetitions();
                logger_1.logger.info('✅ Competitions synced');
                // Step 1.4: Sync Teams (depends on Competitions)
                logger_1.logger.info('⚽ Syncing Teams...');
                await this.teamSyncWorker.syncTeams();
                logger_1.logger.info('✅ Teams synced');
            }
            else {
                logger_1.logger.info(`📦 Database has data. Categories=${categoryCount}, Competitions=${competitionCount}, Teams=${teamCount}`);
                ...
                if (categoryCount === 0) {
                    logger_1.logger.info('📋 Categories missing. Syncing Categories...');
                    try {
                        await this.categorySyncWorker.syncAllCategories();
                        logger_1.logger.info('✅ Categories synced');
                    }
                ...
                if (competitionCount === 0) {
                    logger_1.logger.info('🏆 Competitions missing. Syncing Competitions...');
                    try {
                        await this.competitionSyncWorker.syncCompetitions();
                        logger_1.logger.info('✅ Competitions synced');
                    }
                    catch (error) {
                        logger_1.logger.error('Failed to sync competitions:', error.message);
                    }
                }
                if (teamCount === 0) {
                    logger_1.logger.info('⚽ Teams missing. Syncing Teams...');
                    try {
                        await this.teamSyncWorker.syncTeams();
                        logger_1.logger.info('✅ Teams synced');
                    }
            ...
            logger_1.logger.info('📅 Fetching Today\'s Schedule...');
            await this.syncTodaySchedule();
            logger_1.logger.info('✅ Bootstrap Complete. System Ready. Opening MQTT Gates...');
        }
        catch (error) {
     ...
     */
    async syncTodaySchedule() {
        try {
            const today = new Date();
            const dateStr = (0, timestamp_util_1.formatTheSportsDate)(today).replace(/-/g, ''); // YYYYMMDD format
            logger_1.logger.info(`Fetching match diary for today: ${dateStr}`);
            // Fetch matches from API
            const response = await this.matchDiaryService.getMatchDiary({ date: dateStr });
            if (response.err) {
                logger_1.logger.warn(`Failed to fetch today's schedule: ${response.err}`);
                return;
            }
            if (!response.results || response.results.length === 0) {
                logger_1.logger.info('No matches found for today');
                return;
            }
            logger_1.logger.info(`Found ${response.results.length} matches for today. Syncing...`);
            ...
            if (response.results_extra) {
                logger_1.logger.info('📋 Populating teams and competitions from results_extra...');
                ...
                if (response.results_extra.team) {
                    ...
                    logger_1.logger.info('✅ Teams populated from results_extra');
                }
                ...
                if (response.results_extra.competition) {
                    ...
                    logger_1.logger.info('✅ Competitions populated from results_extra');
                }
            }
            ...
            const matchesToSync = response.results.map((match) => {
            ...
            logger_1.logger.info(`🔄 Starting to sync ${matchesToSync.length} matches in ${totalBatches} batches (${BATCH_SIZE} per batch)...`);
            let totalSynced = 0;
            let totalErrors = 0;
            for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                ...
                try {
                    logger_1.logger.info(`📦 [Bootstrap] Processing batch ${batchNumber}/${totalBatches} (matches ${startIndex + 1}-${endIndex})...`);
                    ...
                    logger_1.logger.info(`✅ [Bootstrap] Batch ${batchNumber}/${totalBatches} completed: ${batchResult.synced} synced, ${batchResult.errors || 0} errors`);
                    ...
                    logger_1.logger.error(`❌ [Bootstrap] Batch ${batchNumber}/${totalBatches} failed:`, error.message);
                    totalErrors += batch.length;
                    // Continue with next batch even if this one failed
                }
            }
            const result = { synced: totalSynced, errors: totalErrors };
            logger_1.logger.info(`✅ Today's schedule synced: ${result.synced}/${matchesToSync.length} matches synced, ${result.errors} errors`);
            // CRITICAL: Log how many matches were actually saved to DB
            if (result.synced < matchesToSync.length) {
                logger_1.logger.warn(`⚠️ Only ${result.synced} out of ${matchesToSync.length} matches were saved. Check for errors.`);
            }
        }
        catch (error) {
            logger_1.logger.error('Failed to sync today\'s schedule:', error);
