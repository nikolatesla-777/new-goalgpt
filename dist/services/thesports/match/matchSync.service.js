...
class MatchSyncService {
    constructor(teamDataService, competitionService) {
     ...
     */
    async syncMatch(matchData, resultsExtra) {
        ...
        try {
            ...
            if (competitionId && competitionId !== '0' && competitionId !== '') {
                if (!this.ensuredCompetitionIds.has(competitionId)) {
                    ...
                    if (!competition || !competition.name) {
                        ...
                        if (resultsExtra?.competition && resultsExtra.competition[competitionId]) {
                            const compData = resultsExtra.competition[competitionId];
                            logger_1.logger.info(`🔧 Creating competition ${competitionId} from results_extra: ${compData.name || compData.name_en || 'Unknown'}`);
                            ...
                            logger_1.logger.info(`✅ Competition ${competitionId} created: ${competition?.name || 'Unknown'}`);
                        }
                        else {
                            // Fallback: Try to fetch from API
                            logger_1.logger.warn(`⚠️ Competition ${competitionId} not in results_extra. Attempting API fetch...`);
                            competition = await this.competitionService.getCompetitionById(competitionId);
                            if (!competition || !competition.name) {
                                logger_1.logger.warn(`⚠️ Competition ${competitionId} still not found. Match will be saved with competition_id but no name.`);
                            }
                        }
                    }
                    ...
                    if (!homeTeam) {
                        ...
                        if (resultsExtra?.team) {
                            if (Array.isArray(resultsExtra.team)) {
                                teamData = resultsExtra.team.find((t) => String(t?.id) === homeTeamId);
                            }
                            else {
                                teamData = resultsExtra.team[homeTeamId];
                            }
                        }
                        if (teamData) {
                            logger_1.logger.info(`🔧 Creating home team ${homeTeamId} from results_extra: ${teamData.name || 'Unknown'}`);
                            // Save to DB immediately using repository
                            ...
                            logger_1.logger.info(`✅ Home team ${homeTeamId} created: ${homeTeam?.name || 'Unknown'}`);
                        }
                        else {
                            // Fallback: Try to fetch from API
                            logger_1.logger.warn(`⚠️ Home team ${homeTeamId} not in results_extra. Attempting API fetch...`);
                            homeTeam = await this.teamDataService.getTeamById(homeTeamId);
                            if (!homeTeam) {
                                logger_1.logger.warn(`⚠️ Home team ${homeTeamId} still not found. Match will be saved with home_team_id but no team data.`);
                            }
                        }
                    }
            ...
            if (awayTeamId && awayTeamId !== '0' && awayTeamId !== '') {
                if (!this.ensuredTeamIds.has(awayTeamId)) {
                    ...
                    if (!awayTeam) {
                        ...
                        if (resultsExtra?.team) {
                            if (Array.isArray(resultsExtra.team)) {
                                teamData = resultsExtra.team.find((t) => String(t?.id) === awayTeamId);
                            }
                            else {
                                teamData = resultsExtra.team[awayTeamId];
                            }
                        }
                        if (teamData) {
                            logger_1.logger.info(`🔧 Creating away team ${awayTeamId} from results_extra: ${teamData.name || 'Unknown'}`);
                            // Save to DB immediately using repository
                            await this.teamRepository.createOrUpdate({
                            ...
                            logger_1.logger.info(`✅ Away team ${awayTeamId} created: ${awayTeam?.name || 'Unknown'}`);
                        }
                        else {
                            // Fallback: Try to fetch from API
                            logger_1.logger.warn(`⚠️ Away team ${awayTeamId} not in results_extra. Attempting API fetch...`);
                            awayTeam = await this.teamDataService.getTeamById(awayTeamId);
                            if (!awayTeam) {
                                logger_1.logger.warn(`⚠️ Away team ${awayTeamId} still not found. Match will be saved with away_team_id but no team data.`);
                            }
                        }
                    }
            ...
            if (matchData.external_id && matchData.external_id.startsWith('l5ergph4')) {
                logger_1.logger.debug(`🔍 [MatchSync] Debug match ${matchData.external_id}:`, {
            ...
            logger_1.logger.debug(`Match ${matchData.external_id} synced successfully`);
        }
        catch (error) {
            await client.query('ROLLBACK');
            logger_1.logger.error(`Failed to sync match ${matchData.external_id}:`, error.message);
            throw error;
        }
        finally {
            client.release();
    ...
    async ensureMatchColumnSupport(client) {
    ...
    `;
        ...
        const cols = new Set(columnCheckResult.rows.map((r) => r.column_name));
        const scoreCols = [
            'home_score_regular', 'home_score_overtime', 'home_score_penalties', 'home_score_display',
            'away_score_regular', 'away_score_overtime', 'away_score_penalties', 'away_score_display',
        ];
        ...
        logger_1.logger.info(`Match schema detected: hasNewScoreColumns=${this.hasNewScoreColumns}, ` +
    ...
    validateMatchData(matchData) {
        ...
        if (validated.match_time) {
            ...
            if (matchTime <= now && minutesDiff > 5 && validated.status_id === enums_1.MatchState.NOT_STARTED) {
                logger_1.logger.warn(`⚠️ [MatchSync] Match ${validated.external_id} kickoff passed (${minutesDiff}m) but status is still NOT_STARTED. ` +
            ...
            if (matchTime > now) {
                const isFinishedState = validated.status_id === enums_1.MatchState.END ||
                    validated.status_id === enums_1.MatchState.CANCEL ||
                    validated.status_id === enums_1.MatchState.INTERRUPT;
                if (isFinishedState) {
                    logger_1.logger.warn(`Match ${validated.external_id} has status ${validated.status_id} (finished) but match_time (${matchDate.toISOString()}) ` +
        ...
        if (validated.match_time && validated.match_time < 1000000000) {
            logger_1.logger.debug(`Match ${validated.external_id} match_time: ${validated.match_time} (Unix seconds)`);
        }
        return validated;
    }
    /**
     ...
     */
    async upsertMatch(client, matchData) {
        ...
        if (hasIncidents && matchData.incidents != null && typeof matchData.incidents === 'string') {
            logger_1.logger.debug(`⚠️ [MatchSync] incidents is string, will parse: ${matchData.incidents.substring(0, 50)}...`);
        }
        if (hasStatistics && matchData.statistics != null && typeof matchData.statistics === 'string') {
            logger_1.logger.debug(`⚠️ [MatchSync] statistics is string, will parse: ${matchData.statistics.substring(0, 50)}...`);
        }
        ...
        let homeScoresValue = null;
        let awayScoresValue = null;
     ...
     * Batch sync matches
     */
    async syncMatches(matches, resultsExtra) {
        let synced = 0;
        let errors = 0;
        const rejectedReasons = {};
        logger_1.logger.info(`🔄 [MatchSync] Starting to sync ${matches.length} matches...`);
        for (let i = 0; i < matches.length; i++) {
            ...
            try {
                ...
                if ((i + 1) % 50 === 0 || i === matches.length - 1) {
                    logger_1.logger.info(`📊 [MatchSync] Progress: ${i + 1}/${matches.length} matches processed (${synced} synced, ${errors} errors)`);
                }
            }
            catch (error) {
                ...
                logger_1.logger.error(`❌ [MatchSync] REJECTED match ${match.external_id}:`, errorDetails);
        ...
        const topReasons = Object.entries(rejectedReasons)
            ...
            .join(', ');
        logger_1.logger.info(`✅ [MatchSync] Completed: ${synced}/${matches.length} matches synced, ${errors} errors`);
        if (errors > 0) {
            logger_1.logger.warn(`⚠️ [MatchSync] ${errors} matches failed to sync. Top rejection reasons: ${topReasons || 'none'}`);
        }
