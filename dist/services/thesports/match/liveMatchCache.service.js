...
class LiveMatchCacheService {
    ...
    getLiveMatches() {
        ...
        this.stats.hits++;
        logger_1.logger.debug(`[LiveMatchCache] HIT - live matches (age: ${age}ms, hits: ${this.liveMatchesCache.hitCount})`);
    ...
    setLiveMatches(data) {
        ...
        logger_1.logger.debug(`[LiveMatchCache] SET - live matches (${data.results?.length || 0} matches)`);
    }
    /**
     * Get cached diary matches for a date
     */
    getDiary(date, statusFilter) {
        ...
        entry.hitCount++;
        this.stats.hits++;
        logger_1.logger.debug(`[LiveMatchCache] HIT - diary ${key} (age: ${age}ms)`);
        return entry.data;
    }
    /**
     * Set diary cache
     */
    setDiary(date, statusFilter, data) {
        const key = this.buildDiaryKey(date, statusFilter);
        ...
        logger_1.logger.debug(`[LiveMatchCache] SET - diary ${key} (${data.results?.length || 0} matches)`);
    }
    /**
     * Get cached unified matches
     */
    getUnified(date, includeLive) {
        ...
        entry.hitCount++;
        this.stats.hits++;
        logger_1.logger.debug(`[LiveMatchCache] HIT - unified ${key} (age: ${age}ms)`);
        return entry.data;
    }
    /**
     * Set unified cache
     */
    setUnified(date, includeLive, data) {
        const key = `${date}:${includeLive}`;
        ...
        logger_1.logger.debug(`[LiveMatchCache] SET - unified ${key} (${data.results?.length || 0} matches)`);
    }
    /**
     ...
     */
    invalidateMatch(matchId, eventType) {
        ...
        if (lastInvalidation && Date.now() - lastInvalidation < this.INVALIDATION_DEDUP_MS) {
            logger_1.logger.debug(`[LiveMatchCache] SKIP invalidation (dedup) - ${matchId} ${eventType}`);
            return;
        }
        ...
        if (this.liveMatchesCache) {
            this.liveMatchesCache = null;
            logger_1.logger.debug(`[LiveMatchCache] INVALIDATE - live matches (${eventType} on ${matchId})`);
        }
    ...
    // Private helpers
    buildDiaryKey(date, statusFilter) {
