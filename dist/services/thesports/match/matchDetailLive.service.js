...
const cache_fallback_util_1 = require("../../../utils/cache/cache-fallback.util");
const IncidentOrchestrator_1 = require("../../orchestration/IncidentOrchestrator");
class MatchDetailLiveService {
    constructor() {
     ...
     */
    async getMatchDetailLive(params, options = {}) {
        const { match_id } = params;
        const cacheKey = `${types_1.CacheKeyPrefix.TheSports}:match:detail_live:${match_id}`;
        if (options.forceRefresh) {
            logger_1.logger.info(`Force-refresh match detail live: ${match_id}`);
            // Phase 4-2: Single circuit layer - circuit breaker is in TheSportsClient
            try {
                return await this.client.get('/match/detail_live', { match_id });
            }
            catch (circuitError) {
        ...
        // Normal cached fetch
        return (0, cache_fallback_util_1.getWithCacheFallback)(cacheKey, async () => {
            logger_1.logger.info(`Fetching match detail live: ${match_id}`);
            // Phase 4-2: Single circuit layer - circuit breaker is in TheSportsClient
            try {
                const response = await this.client.get('/match/detail_live', { match_id });
                ...
                if (isResultEmpty) {
                    logger_1.logger.warn(`[DetailLive] Direct fetch for ${match_id} returned empty. Falling back to global live list search.`);
                    const globalList = await this.getAllLiveStats();
                    const foundInGlobal = globalList.find((m) => String(m?.id || '') === String(match_id) ||
                        String(m?.match_id || '') === String(match_id));
                    if (foundInGlobal) {
                        logger_1.logger.info(`[DetailLive] Found match ${match_id} in global live list fallback!`);
                        // Wrap in expected response structure
                        return { results: [foundInGlobal] };
                    }
                }
                return response;
            }
    ...
    async getMatchDetail(matchId) {
        try {
            ...
            const response = await this.client.get('/match/detail', { match_id: matchId });
            return response;
        }
        catch (error) {
            logger_1.logger.warn(`[MatchDetailLive] Failed to fetch /match/detail for ${matchId}: ${error.message}`);
            return null;
        }
    }
    /**
     ...
     */
    async getAllLiveStats() {
        const cacheKey = `${types_1.CacheKeyPrefix.TheSports}:match:detail_live:all`;
        // Try cache first
        const cached = await (0, cache_fallback_util_1.getWithCacheFallback)(cacheKey, async () => {
            ...
            const response = await this.client.get('/match/detail_live');
     ...
     */
    async getMatchStatsFromLive(matchId) {
        const allLiveMatches = await this.getAllLiveStats();
        // Find the specific match
        const matchData = allLiveMatches.find((m) => m.id === matchId);
        if (matchData) {
            logger_1.logger.info(`Found stats for match ${matchId}: ${matchData.stats?.length || 0} stats, ${matchData.incidents?.length || 0} incidents`);
            return {
                id: matchData.id,
        ...
        logger_1.logger.warn(`Match ${matchId} not found in live data (may not be currently live)`);
        return null;
    }
    extractLiveFields(resp, matchId) {
        ...
        const unwrapResults = (r) => {
            if (!r || typeof r !== 'object')
                return r;
            if (Array.isArray(r)) {
                if (matchId) {
                    // CRITICAL FIX: Check multiple possible ID fields (id, match_id, external_id)
                    const found = r.find((item) => String(item?.id || '') === String(matchId) ||
                        String(item?.match_id || '') === String(matchId) ||
                        String(item?.external_id || '') === String(matchId));
                    if (found) {
                        logger_1.logger.debug(`[DetailLive] matched detail_live by id match_id=${matchId} (len=${r.length})`);
                        return found;
                    }
                    // CRITICAL: If matchId is not found in the array, return null instead of r[0]
                    logger_1.logger.warn(`[DetailLive] match_id=${matchId} not found in detail_live results (len=${r.length}). Checked: id, match_id, external_id`);
                    return null;
                }
                return null;
            }
            ...
            if (keys.length === 1) {
                ...
                if (Array.isArray(v)) {
                    if (matchId) {
                        // CRITICAL FIX: Check multiple possible ID fields (id, match_id, external_id)
                        const found = v.find((item) => String(item?.id || '') === String(matchId) ||
                            String(item?.match_id || '') === String(matchId) ||
                            String(item?.external_id || '') === String(matchId));
                        if (found) {
                            logger_1.logger.debug(`[DetailLive] matched detail_live by id match_id=${matchId} (len=${v.length}, key=${keys[0]})`);
                            return found;
                        }
                        // CRITICAL: If matchId is not found in the array, return null instead of v[0]
                        logger_1.logger.warn(`[DetailLive] match_id=${matchId} not found in detail_live results (len=${v.length}, key=${keys[0]}). Checked: id, match_id, external_id`);
                        return null;
                    }
                    return null;
                }
                ...
                if (Array.isArray(v)) {
                    if (matchId) {
                        ...
                        const found = v.find((item) => String(item?.id || '') === String(matchId) ||
                            ...
                            String(item?.external_id || '') === String(matchId));
                        if (found) {
                            logger_1.logger.debug(`[DetailLive] matched detail_live by id match_id=${matchId} (len=${v.length}, key=1)`);
                            return found;
                        }
                        // CRITICAL: If matchId is not found in the array, return null instead of v[0]
                        logger_1.logger.warn(`[DetailLive] match_id=${matchId} not found in detail_live results (len=${v.length}, key=1). Checked: id, match_id, external_id`);
                        return null;
                    }
                    return null;
                }
        ...
        if (Array.isArray(root?.score) && root.score.length >= 2 && typeof root.score[1] === 'number') {
            ...
            logger_1.logger.debug(`[DetailLive] Extracted status_id=${statusId} from score array format`);
        }
        else {
            statusId =
        ...
        if (Array.isArray(root?.score) && root.score.length >= 4) {
            ...
            if (homeScoreDisplay !== null || awayScoreDisplay !== null) {
                logger_1.logger.debug(`[DetailLive] Extracted scores from score array format: ${homeScoreDisplay}-${awayScoreDisplay}`);
            }
        }
        ...
        const incidents = (Array.isArray(root?.incidents) ? root.incidents : null) ??
            (Array.isArray(root?.events) ? root.events : null) ??
            (Array.isArray(root?.match_incidents) ? root.match_incidents : null) ??
            null;
        const statistics = (Array.isArray(root?.statistics) ? root.statistics : null) ??
        ...
        if (Array.isArray(root?.score) && root.score.length >= 5 && typeof root.score[4] === 'number') {
            ...
            logger_1.logger.debug(`[DetailLive] Extracted kick-off timestamp=${liveKickoffTimeRaw} from score array format (index 4)`);
        }
        else {
        ...
        const liveKickoffTime = typeof liveKickoffTimeRaw === 'number' && Number.isFinite(liveKickoffTimeRaw) && liveKickoffTimeRaw > 0
            ? liveKickoffTimeRaw
            : null;
        const updateTime = typeof updateTimeRaw === 'number' && Number.isFinite(updateTimeRaw) && updateTimeRaw > 0
            ? updateTimeRaw
            : null;
        ...
        if (updateTime === null && matchId) {
            const tliveInfo = root?.tlive !== undefined
                ...
                : 'tlive=undefined';
            logger_1.logger.warn(`[DetailLive] update_time NOT FOUND for match ${matchId}. ${tliveInfo}. Checked fields: tlive, update_time, updateTime, updated_at, match.update_time. Root keys: ${Object.keys(root || {}).join(', ')}`);
        }
        // Extract minute from provider (WebSocket/detail_live source)
        const minuteRaw = (typeof root?.minute === 'number' ? root.minute : null) ??
            ...
            null;
        const minute = typeof minuteRaw === 'number' && Number.isFinite(minuteRaw) && !isNaN(minuteRaw) && minuteRaw >= 0
            ? minuteRaw
            : null;
     ...
     */
    calculateMinuteFromKickoffs(statusId, firstHalfKickoffTs, secondHalfKickoffTs, overtimeKickoffTs, existingMinute, nowTs) {
        if (statusId === null)
            return null;
        // Status 2 (FIRST_HALF)
        if (statusId === 2) {
     ...
     */
    async ensureReconcileSchema(client) {
    ...
    `);
        ...
        logger_1.logger.info(`📐 [DetailLive] Reconcile schema: incidents=${this.hasIncidentsColumn}, statistics=${this.hasStatisticsColumn}, newScoreColumns=${this.hasNewScoreColumns}, liveKickoff=${this.hasLiveKickoffTimeColumn}, minuteCol=${this.minuteColumnName ?? 'none'}`);
    }
    /**
     ...
     */
    async reconcileMatchToDatabase(match_id, providerUpdateTimeOverride = null) {
        const t0 = Date.now();
        ...
        if (results && Array.isArray(results)) {
            const foundMatch = results.find((m) => String(m?.id || m?.match_id) === String(match_id));
            if (foundMatch) {
                ...
                if (providerStatus === 8 && live.statusId !== 8) {
                    logger_1.logger.warn(`[DetailLive] CRITICAL: Provider says END (8) for ${match_id} but extractLiveFields returned statusId=${live.statusId}. ` +
                    ...
                    if (Array.isArray(foundMatch.score) && foundMatch.score.length >= 2) {
                        ...
                        logger_1.logger.info(`[DetailLive] Fixed extraction: statusId=${live.statusId}, score=${live.homeScoreDisplay}-${live.awayScoreDisplay}`);
                    }
                }
            }
        }
        ...
        try {
            ...
            const existingResult = await client.query(`SELECT provider_update_time, last_event_ts, status_id, match_time,
         ...
         FROM ts_matches WHERE external_id = $1`, [match_id]);
            if (existingResult.rows.length === 0) {
                ...
                let apiMatch = null;
                if (results && Array.isArray(results)) {
                    apiMatch = results.find((m) => String(m?.id || m?.match_id) === String(match_id));
                }
                ...
                let matchTime = Math.floor(Date.now() / 1000);
                let statusId = live.statusId || 1;
                if (apiMatch) {
                ...
                if (!homeTeamId || !awayTeamId || !competitionId) {
                    logger_1.logger.info(`[DetailLive] Match ${match_id} missing metadata (home=${homeTeamId}, away=${awayTeamId}, comp=${competitionId}). Fetching from /match/detail...`);
                    try {
                        ...
                        if (detailMatch && !detailMatch.err) {
                            ...
                            logger_1.logger.info(`[DetailLive] Got metadata from /match/detail: home=${homeTeamId}, away=${awayTeamId}, comp=${competitionId}`);
                        }
                    }
                    catch (fetchErr) {
                        logger_1.logger.warn(`[DetailLive] Failed to fetch /match/detail for ${match_id}: ${fetchErr.message}`);
                    }
                }
                ...
                let diaryHomeScore = null;
                let diaryAwayScore = null;
                if (!homeTeamId || !awayTeamId) {
                    logger_1.logger.info(`[DetailLive] Match ${match_id} still missing team IDs after /match/detail. Trying /match/diary...`);
                    try {
                        // Get today's date in YYYYMMDD format
                        const today = new Date().toISOString().slice(0, 10).replace(/-/g, '');
                        const diaryResp = await this.client.get('/match/diary', { date: today });
                        const diaryResults = diaryResp?.results || [];
                        const diaryMatch = diaryResults.find((m) => String(m?.id) === String(match_id));
                        if (diaryMatch) {
                            ...
                            logger_1.logger.info(`[DetailLive] ✅ Got metadata from /match/diary: home=${homeTeamId}, away=${awayTeamId}, comp=${competitionId}, score=${diaryHomeScore ?? '?'}-${diaryAwayScore ?? '?'}, status=${statusId}`);
                        }
                        else {
                            logger_1.logger.warn(`[DetailLive] Match ${match_id} not found in /match/diary for date ${today}`);
                        }
                    }
                    catch (diaryErr) {
                        logger_1.logger.warn(`[DetailLive] Failed to fetch /match/diary: ${diaryErr.message}`);
                    }
                }
                // Validate we have minimum required data for insert
                if (homeTeamId && awayTeamId) {
                    logger_1.logger.info(`[DetailLive] Match ${match_id} not found in DB. Auto-inserting with home=${homeTeamId}, away=${awayTeamId}, comp=${competitionId}...`);
                    try {
             ...
             `, [
                        ...
                        if (insertRes.rowCount != null && insertRes.rowCount > 0) {
                            logger_1.logger.info(`[DetailLive] ✅ Successfully auto-inserted match ${match_id} (home=${homeTeamId}, away=${awayTeamId})`);
                            return { updated: true, rowCount: 1, statusId: statusId, score: null, providerUpdateTime: null };
                        }
                    }
                    catch (err) {
                        // Log detailed error for debugging
                        logger_1.logger.error(`[DetailLive] Failed to auto-insert match ${match_id}: ${err.message}. Home=${homeTeamId}, Away=${awayTeamId}, Comp=${competitionId}`);
                        return { updated: false, rowCount: 0, statusId: live.statusId, score: null, providerUpdateTime: null };
                    }
                }
                else {
                    logger_1.logger.warn(`[DetailLive] Match ${match_id} not found in DB, and could not resolve team IDs (home=${homeTeamId}, away=${awayTeamId}). Skipping auto-insert.`);
                }
            ...
            if (live.statusId === null && live.homeScoreDisplay === null && live.awayScoreDisplay === null) {
                ...
                const results = resp.results || resp.result_list;
                if (results) {
                    logger_1.logger.warn(`[DetailLive] Match ${match_id} not found in detail_live response. ` +
                ...
                if (providerUpdateTimeOverride !== null) {
                    logger_1.logger.info(`[DetailLive] No usable data for ${match_id} but providerUpdateTimeOverride provided, ` +
                    ...
                    if (nowTs >= minTimeForEnd) {
                        ...
                        logger_1.logger.info(`[DetailLive] Match ${match_id} not found in provider response and match_time (${matchTime}) ` +
                            `is ${Math.floor((nowTs - matchTime) / 60)} minutes ago (>150 min). ` +
                            `Transitioning from status ${existingStatusId} to END (8).`);
                        const updateResult = await client.query(`UPDATE ts_matches 
               ...
               WHERE external_id = $2 AND status_id IN (2, 3, 4, 5, 7)`, [nowTs, match_id]);
                        ...
                        logger_1.logger.info(`[DetailLive] Match ${match_id} not found in provider response but match_time (${matchTime}) ` +
            ...
            if (existingEventTime !== null && (existingEventTime > MAX_VALID_TS || existingEventTime < MIN_VALID_TS)) {
                logger_1.logger.warn(`[DetailLive] CORRUPTED last_event_ts detected for ${match_id}: ${existingEventTime} ` +
            ...
            if (existingProviderTime !== null && (existingProviderTime > MAX_VALID_TS || existingProviderTime < MIN_VALID_TS)) {
                logger_1.logger.warn(`[DetailLive] CORRUPTED provider_update_time detected for ${match_id}: ${existingProviderTime} ` +
            ...
            if (!isCriticalTransition) {
                if (incomingProviderUpdateTime !== null && incomingProviderUpdateTime !== undefined) {
                    ...
                    if (validExistingProviderTime !== null && incomingProviderUpdateTime <= validExistingProviderTime) {
                        logger_1.logger.debug(`Skipping stale update for ${match_id} (provider time: ${incomingProviderUpdateTime} <= ${validExistingProviderTime})`);
                        return { updated: false, rowCount: 0, statusId: live.statusId, score: null, providerUpdateTime: null };
                    }
                }
                else {
                    ...
                    if (existingEventTime !== null && ingestionTs <= existingEventTime + 5) {
                        logger_1.logger.debug(`Skipping stale update for ${match_id} (event time: ${ingestionTs} <= ${existingEventTime + 5})`);
                        return { updated: false, rowCount: 0, statusId: live.statusId, score: null, providerUpdateTime: null };
                    }
                }
            }
                ...
                logger_1.logger.info(`[DetailLive] ⚡ CRITICAL TRANSITION (${transitionType}) for ${match_id}: ` +
                ...
                if (existingStatusId === 2 && live.statusId === 3) {
                    logger_1.logger.info(`[DetailLive] ⚽ HALF_TIME TRANSITION! Saving first half stats for ${match_id}`);
                    this.saveFirstHalfStatsOnHalftime(match_id).catch(err => {
                        logger_1.logger.error(`[DetailLive] Failed to save first half stats for ${match_id}:`, err);
                    });
                }
            }
            // Calculate provider_update_time to write (max of existing and incoming)
            const providerTimeToWrite = incomingProviderUpdateTime !== null && incomingProviderUpdateTime !== undefined
        ...
        END`,
            ...
            const values = [providerTimeToWrite, ingestionTs];
            let i = 3;
            ...
            if (hasLiveData && live.statusId !== null) {
                ...
                if ((live.statusId === 2 || live.statusId === 3 || live.statusId === 4 || live.statusId === 5 || live.statusId === 7) && existing.first_half_kickoff_ts === null) {
                    ...
                    if (live.liveKickoffTime !== null) {
                        ...
                        const existingMatchTimeResult = await client.query(`SELECT match_time FROM ts_matches WHERE external_id = $1`, [match_id]);
                    ...
                    logger_1.logger.info(`[KickoffTS] set first_half_kickoff_ts=${finalKickoffTime} match_id=${match_id} source=${source} status=${live.statusId}`);
                }
                    ...
                    if (existing.first_half_kickoff_ts === null) {
                        ...
                        logger_1.logger.debug(`[KickoffTS] skip (already set) first_half_kickoff_ts match_id=${match_id}`);
                    }
                }
                ...
                if (hasLiveData && live.statusId === 4 && live.liveKickoffTime !== null) {
                    ...
                    logger_1.logger.info(`[KickoffTS] set second_half_kickoff_ts=${live.liveKickoffTime} match_id=${match_id} source=provider_score_array status=${live.statusId}`);
                }
                    ...
                    if (existing.first_half_kickoff_ts !== null) {
                        ...
                        logger_1.logger.info(`[KickoffTS] Estimating second_half_kickoff_ts=${secondHalfKickoffValue} from first_half_kickoff_ts=${existing.first_half_kickoff_ts} for match_id=${match_id}`);
                    }
                    setParts.push(`second_half_kickoff_ts = $${i++}`);
                    values.push(secondHalfKickoffValue);
                    secondHalfKickoffToUse = secondHalfKickoffValue;
                    logger_1.logger.info(`[KickoffTS] set second_half_kickoff_ts=${secondHalfKickoffValue} match_id=${match_id} source=estimated existing_status=${existingStatusId}`);
                }
                ...
                if (live.statusId === 5 && existing.overtime_kickoff_ts === null) {
                    ...
                    logger_1.logger.info(`[KickoffTS] set overtime_kickoff_ts=${kickoffTimeToUse} match_id=${match_id} source=${source} existing_status=${existingStatusId}`);
                }
                else if (live.statusId === 5 && existing.overtime_kickoff_ts !== null) {
                    logger_1.logger.debug(`[KickoffTS] skip (already set) overtime_kickoff_ts match_id=${match_id}`);
                }
            }
            ...
            if (this.minuteColumnName) {
                if (live.minute !== null) {
                    ...
                    logger_1.logger.debug(`[DetailLive] Setting minute=${live.minute} from provider for match_id=${match_id}`);
                }
                else {
                    ...
                    if (effectiveFirstHalfKickoff === null && live.statusId === 2) {
                        ...
                        logger_1.logger.warn(`[DetailLive] first_half_kickoff_ts is NULL for status 2 match ${match_id}, using ingestionTs=${ingestionTs} for minute calculation`);
                    }
                    ...
                    if (isHalfTime || isFinished) {
                        ...
                        logger_1.logger.info(`[DetailLive] Terminal status ${live.statusId} detected for match_id=${match_id}. Forcing minute=NULL`);
                    }
                    else {
                        // For live matches, calculate minute from kickoff timestamps
                        const calculatedMinute = this.calculateMinuteFromKickoffs(live.statusId, effectiveFirstHalfKickoff, // Use new value if we just set it, otherwise existing, or ingestionTs fallback
                        ...
                        if (calculatedMinute !== null) {
                            ...
                            logger_1.logger.info(`[DetailLive] Setting calculated minute=${calculatedMinute} from kickoff_ts for match_id=${match_id} status=${live.statusId} (first_half_ts=${effectiveFirstHalfKickoff}, second_half_ts=${secondHalfKickoffToUse})`);
                        }
                        else {
                            // CRITICAL: If we can't calculate minute but match is live, log warning
                            logger_1.logger.warn(`[DetailLive] Cannot calculate minute for match_id=${match_id} status=${live.statusId} - kickoff timestamps missing (first_half=${effectiveFirstHalfKickoff}, second_half=${secondHalfKickoffToUse})`);
                        }
                    }
                }
            }
            ...
            if (this.hasIncidentsColumn && live.incidents !== null) {
                ...
                try {
                    ...
                    logger_1.logger.warn(`[DetailLive] IncidentOrchestrator failed for ${match_id}:`, incErr);
      ...
      `;
            ...
            if (affected === 0) {
                logger_1.logger.warn(`⚠️ [DetailLive] UPDATE affected 0 rows: matchId=${match_id}, ` +
            ...
            const updatedRowResult = await client.query('SELECT provider_update_time FROM ts_matches WHERE external_id = $1', [match_id]);
     ...
     */
    async saveFirstHalfStatsOnHalftime(matchId) {
        try {
            // Import CombinedStatsService dynamically to avoid circular dependency
            const { CombinedStatsService } = await Promise.resolve().then(() => __importStar(require('./combinedStats.service')));
            ...
            if (hasStats) {
                logger_1.logger.debug(`[DetailLive] First half stats already saved for ${matchId}, skipping`);
                return;
            }
            ...
            if (result && result.allStats && result.allStats.length > 0) {
                ...
                logger_1.logger.info(`[DetailLive] ✅ Saved first half stats for ${matchId} (${result.allStats.length} stats)`);
            }
            else {
                logger_1.logger.warn(`[DetailLive] No stats available to save for ${matchId} at halftime`);
            }
        }
        catch (error) {
            logger_1.logger.error(`[DetailLive] Error saving first half stats for ${matchId}:`, error);
        }
    }
}
