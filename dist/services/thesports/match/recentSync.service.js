...
exports.RecentSyncService = void 0;
...
const matchSync_service_1 = require("./matchSync.service");
...
class RecentSyncService {
    constructor(matchSyncService) {
        ...
        if (matchSyncService) {
            ...
            this.matchSyncService = new matchSync_service_1.MatchSyncService(teamDataService, competitionService);
        }
    }
    /**
     ...
     */
    async syncIncremental(lastSyncTimestamp) {
        return (0, sync_strategy_1.withSyncLock)(sync_strategy_1.SyncType.RECENT, async () => {
            ...
            if (lastSyncTimestamp) {
                ...
                if (syncState && syncState.last_updated_at) {
                    ...
                    logger_1.logger.info('[RecentSync] First sync detected - will fetch ALL pages');
                }
            }
            const currentUnix = (0, timestamp_util_1.getCurrentUnixTimestamp)();
            logger_1.logger.info(`[RecentSync] Incremental sync: ${lastSyncUnix} -> ${currentUnix} (time param: ${lastSyncUnix + 1})`);
            ...
            while (hasMore && page <= maxPages) {
                const response = await this.client.get('/match/recent/list', {
                    page,
                    limit,
                ...
                // Check for errors
                if (response.err) {
                    logger_1.logger.warn(`[RecentSync] TheSports API error for match/recent/list page ${page}: ${response.err}`);
                    break;
                }
                ...
                if (results.length === 0 || total === 0) {
                    ...
                    hasMore = false;
                    logger_1.logger.debug(`[RecentSync] Page ${page}: No more results (total=${total})`);
                }
                else {
                    allResults = allResults.concat(results);
                    logger_1.logger.info(`[RecentSync] Page ${page}: Fetched ${results.length} matches (total so far: ${allResults.length})`);
            ...
            if (page > maxPages) {
                logger_1.logger.warn(`[RecentSync] Hit max page limit (${maxPages}). Some matches may be missed.`);
            }
            logger_1.logger.info(`[RecentSync] Found ${allResults.length} changed matches since last sync (time=${lastSyncUnix + 1})`);
            if (allResults.length === 0) {
                logger_1.logger.debug('[RecentSync] No matches to sync');
            ...
            for (const match of allResults) {
                try {
                    const matchData = this.mapApiMatchToSyncData(match);
                    await this.matchSyncService.syncMatch(matchData);
                    synced++;
                }
                catch (error) {
                    logger_1.logger.error(`[RecentSync] Failed to sync match ${match.id || 'unknown'}:`, error.message);
                    errors++;
                }
            }
            ...
            if (allResults.length > 0) {
                ...
                if (maxUpdatedAt > 0) {
                    ...
                    logger_1.logger.info(`[RecentSync] Updated sync state to ${maxUpdatedAt} (${new Date(maxUpdatedAt * 1000).toISOString()})`);
                }
            }
            logger_1.logger.info(`[RecentSync] Match incremental sync completed: ${synced} synced, ${errors} errors`);
...
exports.RecentSyncService = RecentSyncService;
