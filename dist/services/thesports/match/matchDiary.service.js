...
class MatchDiaryService {
    ...
    async getMatchDiary(params = {}) {
        ...
        if (params.date) {
            ...
            if (!/^\d{8}$/.test(dateStr)) {
                logger_1.logger.warn(`Invalid date format: ${params.date}. Expected YYYYMMDD format.`);
                ...
                if (params.date.length === 8 && /^\d{8}$/.test(params.date)) {
                    ...
                    logger_1.logger.warn(`Using today's date instead: ${dateStr}`);
                }
            }
        }
        else {
        ...
        if (!/^\d{8}$/.test(dateStr)) {
            logger_1.logger.error(`Date format validation failed: ${dateStr}. Using today's date.`);
            dateStr = (0, timestamp_util_1.getTSIDateString)(); // Use TSI timezone
        }
        ...
        if (!params.forceRefresh) {
            ...
            if (cached) {
                logger_1.logger.debug(`Cache hit for match diary: ${cacheKey}`);
                return cached;
            }
        }
        else {
        ...
        logger_1.logger.info(`🔍 [MatchDiary] Fetching match diary for date: ${dateStr} (format: YYYYMMDD) with pagination`);
        let allResults = [];
        let page = 1;
        const limit = 500; // Recommended limit per page
        const maxPages = 20; // Safety limit to prevent infinite loops
        let hasMore = true;
        let resultsExtra = undefined;
        while (hasMore && page <= maxPages) {
            const response = await this.client.get('/match/diary', { date: dateStr, page, limit });
            // Check for errors
            if (response.err) {
                logger_1.logger.warn(`[MatchDiary] TheSports API error for diary page ${page}: ${response.err}`);
                break;
            }
            // Check API error codes
            if (response?.code && response.code !== 200 && response.code !== 0) {
                const errorMsg = response.msg || 'TheSports API error';
                logger_1.logger.warn(`[MatchDiary] API error for diary page ${page}: ${errorMsg}`);
                break;
            }
            ...
            if (results.length === 0) {
                ...
                hasMore = false;
                logger_1.logger.debug(`[MatchDiary] Page ${page}: No more results (${results.length} results, total=${total})`);
            }
            else {
                allResults = allResults.concat(results);
                logger_1.logger.info(`[MatchDiary] Page ${page}: Fetched ${results.length} matches (total so far: ${allResults.length})`);
        ...
        if (page > maxPages) {
            logger_1.logger.warn(`[MatchDiary] Hit max page limit (${maxPages}). Some matches may be missed.`);
        }
        logger_1.logger.info(`📊 [MatchDiary] Total fetched: ${allResults.length} matches for date ${dateStr}`);
        // Check if no results found
        if (allResults.length === 0) {
            logger_1.logger.warn(`⚠️ [MatchDiary] No matches found for date ${dateStr}. This might be normal if no matches are scheduled.`);
            return { results: [], err: undefined };
        }
        else if (allResults.length < 50) {
            logger_1.logger.warn(`⚠️ [MatchDiary] Only ${allResults.length} matches found. Expected 200+ for a full day.`);
        }
        else {
            logger_1.logger.info(`✅ [MatchDiary] Good match count: ${allResults.length} matches`);
        }
        ...
        if (response.results_extra?.team) {
            ...
            logger_1.logger.info(`📋 Extracted ${teamNamesMap.size} team names from results_extra (format: ${Array.isArray(response.results_extra.team) ? 'ARRAY' : 'OBJECT'})`);
        }
        ...
        const results = (response.results || []).map((match) => {
            ...
            if (competitionId && response.results_extra?.competition) {
                const compData = response.results_extra.competition[competitionId];
                if (compData) {
                    competitionInfo = {
                        id: competitionId,
        ...
        if (finalResults.length > 0) {
            const sampleFinal = finalResults[0];
        ...
        const matchesWithCompetition = finalResults.filter((m) => m.competition && m.competition.name).length;
        const matchesWithCompetitionInfo = finalResults.filter((m) => m.competition_info).length;
        const coveragePct = finalResults.length > 0 ? Math.round((matchesWithCompetition / finalResults.length) * 100) : 0;
        logger_1.logger.info(`📊 [MatchDiary] Final results: ${finalResults.length} matches, ` +
        ...
        finalResults.forEach((match) => {
            if (match.competition_id) {
                if (!match.competition) {
                    logger_1.logger.warn(`⚠️ Match ${match.id} has competition_id ${match.competition_id} but no competition object. Creating placeholder.`);
                    match.competition = {
                        id: match.competition_id,
                        name: null,
                        logo_url: null,
        ...
        // Cache full response
        await cache_service_1.cacheService.set(cacheKey, enrichedResponse, types_1.CacheTTL.Day);
        return enrichedResponse;
    }
    /**
     * Cache results_extra separately for team data enrichment
     */
    async cacheResultsExtra(dateStr, resultsExtra) {
        if (!resultsExtra)
            return;
        const extraCacheKey = `${types_1.CacheKeyPrefix.TheSports}:diary:extra:${dateStr}`;
        await cache_service_1.cacheService.set(extraCacheKey, resultsExtra, types_1.CacheTTL.Day);
        logger_1.logger.debug(`Cached results_extra for date: ${dateStr}`);
    }
    /**
     * Build cache key from date
     */
    buildCacheKey(dateStr) {
        return `${types_1.CacheKeyPrefix.TheSports}:match:diary:${dateStr}`;
    }
}
