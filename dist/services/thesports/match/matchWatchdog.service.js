...
exports.MatchWatchdogService = void 0;
const connection_1 = require("../../../database/connection");
const logger_1 = require("../../../utils/logger");
class MatchWatchdogService {
    /**
     ...
     */
    async findStaleLiveMatches(nowTs, staleSeconds = 120, halfTimeStaleSeconds = 300, // CRITICAL FIX (2026-01-15): Reduced from 900 to 300
    limit = 50) {
      ...
      `;
            ...
            const staleMatches = rows.map((row) => {
            ...
            logger_1.logger.error('[Watchdog] Error finding stale matches:', error);
            throw error;
        }
        finally {
            client.release();
     ...
     */
    async findOverdueMatches(limit = 50) {
        const client = await connection_1.pool.connect();
        try {
      ...
      `;
            ...
            logger_1.logger.error('[Watchdog] Error finding overdue matches:', error);
            throw error;
        }
        finally {
            client.release();
     ...
     */
    async findShouldBeLiveMatches(nowTs, maxMinutesAgo = 1440, limit = 50) {
        const client = await connection_1.pool.connect();
        try {
     ...
     */
    async findImmediateKickoffs(nowTs, windowSeconds = 120, limit = 100) {
        const client = await connection_1.pool.connect();
        try {
            const query = `
        SELECT 
      ...
      `;
            ...
            logger_1.logger.error('[Watchdog] Error finding immediate kickoffs:', error);
            return [];
        }
        finally {
            client.release();
     ...
     */
    async findOverdueFinishes(nowTs, limit = 100) {
        const client = await connection_1.pool.connect();
        try {
      ...
      `;
            ...
            logger_1.logger.error('[Watchdog] Error finding overdue finishes:', error);
            return [];
        }
        finally {
            client.release();
...
exports.MatchWatchdogService = MatchWatchdogService;
