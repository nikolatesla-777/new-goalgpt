...
const matchDetailLive_service_1 = require("./matchDetailLive.service");
const matchTeamStats_service_1 = require("./matchTeamStats.service");
...
class CombinedStatsService {
     ...
     */
    async getCombinedMatchStats(matchId) {
        const cacheKey = `${types_1.CacheKeyPrefix.TheSports}:match:combined_stats:${matchId}`;
        // Check cache first (short TTL for live data)
        const cached = await cache_service_1.cacheService.get(cacheKey);
        if (cached) {
            logger_1.logger.debug(`Cache hit for combined stats: ${matchId}`);
            return cached;
        }
        logger_1.logger.info(`Fetching combined stats for match: ${matchId}`);
        ...
        if (liveStatsResult.status === 'fulfilled' && liveStatsResult.value) {
            ...
            if (Array.isArray(liveData.stats) && liveData.stats.length > 0) {
                basicStats = liveData.stats.map((stat) => {
                    const typeInfo = exports.STAT_TYPE_MAP[stat.type] || { name: `Unknown (${stat.type})`, nameTr: `Bilinmiyor (${stat.type})` };
                    return {
                        type: stat.type,
                        home: stat.home ?? 0,
                        away: stat.away ?? 0,
            ...
            logger_1.logger.info(`Live stats for ${matchId}: ${basicStats.length} basic stats, ${incidents.length} incidents`);
        }
        else if (liveStatsResult.status === 'rejected') {
            logger_1.logger.warn(`Failed to fetch live stats for ${matchId}: ${liveStatsResult.reason}`);
        }
        // Fallback: If no live stats found (e.g. match finished), try historical endpoint
        if (!isLiveStatsFound) {
            logger_1.logger.info(`No live stats found for ${matchId}, attempting fallback to historical data`);
            try {
                const historyData = await this.client.get('/match/live/history', { match_id: matchId });
                const results = historyData?.results || [];
                if (Array.isArray(results) && results.length > 0) {
                    const matchData = results.find((r) => r.id === matchId || r.match_id === matchId) || results[0];
                    ...
                    if (Array.isArray(statsArray)) {
                        basicStats = statsArray.map((stat) => {
                            const typeInfo = exports.STAT_TYPE_MAP[stat.type] || { name: `Unknown (${stat.type})`, nameTr: `Bilinmiyor (${stat.type})` };
                            return {
                                type: stat.type,
                                home: stat.home ?? 0,
                        ...
                        logger_1.logger.info(`Fallback successful: found ${basicStats.length} basic stats in history for ${matchId}`);
                    }
                }
                else {
                    logger_1.logger.info(`No historical data found for ${matchId}`);
                }
            }
            catch (error) {
                logger_1.logger.warn(`Failed to fetch historical fallback for ${matchId}: ${error}`);
            }
        }
        ...
        if (teamStatsResult.status === 'fulfilled' && teamStatsResult.value) {
            ...
            logger_1.logger.info(`Raw team stats results count: ${results.length}`);
            if (Array.isArray(results) && results.length > 0) {
                // Find the specific match data
                const matchData = results.find((r) => r.id === matchId || r.match_id === matchId);
                if (matchData) {
                    logger_1.logger.info(`Found match data for ${matchId}. Keys: ${Object.keys(matchData).join(',')}`);
                    ...
                    // Default fallback if not team stats
                        ...
                        detailedStats = matchData.stats.map((stat) => {
                            const typeInfo = exports.STAT_TYPE_MAP[stat.type] || { name: `Unknown (${stat.type})`, nameTr: `Bilinmiyor (${stat.type})` };
                            return {
                                type: stat.type,
                                home: stat.home ?? 0,
                    ...
                    logger_1.logger.warn(`[STATS_DEBUG] Match data not found in results for ${matchId}`);
                }
            }
            logger_1.logger.info(`Team stats for ${matchId}: ${detailedStats.length} detailed stats`);
        }
        else if (teamStatsResult.status === 'rejected') {
            logger_1.logger.warn(`Failed to fetch team stats for ${matchId}: ${teamStatsResult.reason}`);
        }
        ...
        await cache_service_1.cacheService.set(cacheKey, result, types_1.CacheTTL.TenSeconds * 3);
        return result;
    }
    /**
     ...
     */
    async getHistoricalMatchStats(matchId) {
        const cacheKey = `${types_1.CacheKeyPrefix.TheSports}:match:historical_stats:${matchId}`;
        // Check cache first (longer TTL for historical data)
        const cached = await cache_service_1.cacheService.get(cacheKey);
        if (cached) {
            logger_1.logger.debug(`Cache hit for historical stats: ${matchId}`);
            return cached;
        }
        logger_1.logger.info(`Fetching historical stats for match: ${matchId}`);
        ...
        if (historyResult.status === 'fulfilled' && historyResult.value) {
            ...
            logger_1.logger.info(`Historical stats for ${matchId}: ${basicStats.length} basic stats`);
        }
        else if (historyResult.status === 'rejected') {
            logger_1.logger.warn(`Failed to fetch historical stats for ${matchId}: ${historyResult.reason}`);
        }
        ...
        if (teamStatsResult.status === 'fulfilled' && teamStatsResult.value) {
            ...
            if (Array.isArray(results) && results.length > 0) {
                ...
                if (Array.isArray(statsArray)) {
                    detailedStats = statsArray.map((stat) => {
                        const typeInfo = exports.STAT_TYPE_MAP[stat.type] || { name: `Unknown (${stat.type})`, nameTr: `Bilinmiyor (${stat.type})` };
                        return {
                            type: stat.type,
                            home: stat.home ?? 0,
        ...
        await cache_service_1.cacheService.set(cacheKey, result, types_1.CacheTTL.FiveMinutes);
        return result;
    }
    /**
     * Transform named stats from team_stats/list (realtime) into standard stat objects
     */
    transformTeamStats(teamsStats) {
        ...
        for (const [field, typeId] of Object.entries(FIELD_TO_ID_MAP)) {
            ...
            if (homeValue > 0 || awayValue > 0 || ['passes', 'fouls', 'offsides'].includes(field)) {
                const typeInfo = exports.STAT_TYPE_MAP[typeId] || { name: `Unknown (${typeId})`, nameTr: `Bilinmiyor (${typeId})` };
                result.push({
                    type: typeId,
                    home: homeValue,
                    away: awayValue,
     ...
     */
    async saveCombinedStatsToDatabase(matchId, stats, halfTimeStats) {
        ...
        try {
            ...
            if (columnCheck.rows.length === 0) {
                logger_1.logger.warn(`[CombinedStats] statistics column does not exist, skipping save for ${matchId}`);
                return;
            }
            ...
            logger_1.logger.info(`[CombinedStats] Saved combined stats to database for match: ${matchId} (half-time: ${!!statisticsData.half_time_stats})`);
        }
        catch (error) {
            logger_1.logger.error(`[CombinedStats] Error saving stats to database for ${matchId}:`, error);
     ...
     */
    async saveHalfTimeStatsToDatabase(matchId, halfTimeStats) {
        const client = await connection_1.pool.connect();
        try {
            ...
            await client.query(`
                ...
                WHERE external_id = $2
            `, [JSON.stringify(statisticsData), matchId]);
            logger_1.logger.info(`[CombinedStats] Saved half-time stats to database for match: ${matchId}`);
        }
        catch (error) {
            logger_1.logger.error(`[CombinedStats] Error saving half-time stats to database for ${matchId}:`, error);
        }
        finally {
            client.release();
        }
     ...
     */
    async getCombinedStatsFromDatabase(matchId) {
        const client = await connection_1.pool.connect();
        try {
            ...
            if (!statsData.all_stats || !Array.isArray(statsData.all_stats)) {
                logger_1.logger.warn(`[CombinedStats] Invalid statistics structure in DB for ${matchId}`);
                return null;
            }
            return {
            ...
            logger_1.logger.error(`[CombinedStats] Error reading stats from database for ${matchId}:`, error);
            return null;
        }
        finally {
            client.release();
     ...
     */
    async getHalfTimeStatsFromDatabase(matchId) {
        const client = await connection_1.pool.connect();
        try {
            const result = await client.query(`
            ...
            logger_1.logger.error(`[CombinedStats] Error reading half-time stats from database for ${matchId}:`, error);
            return null;
        }
        finally {
            client.release();
     ...
     */
    async saveFirstHalfStats(matchId, stats) {
        const client = await connection_1.pool.connect();
        try {
            await client.query(`
                UPDATE ts_matches
            ...
            logger_1.logger.info(`[CombinedStats] ✅ Saved first half stats for match: ${matchId} (${stats.length} stats)`);
        }
        catch (error) {
            logger_1.logger.error(`[CombinedStats] Error saving first half stats for ${matchId}:`, error);
        }
        finally {
            client.release();
        }
     ...
     */
    async getFirstHalfStats(matchId) {
        const client = await connection_1.pool.connect();
        try {
            const result = await client.query(`
            ...
            logger_1.logger.error(`[CombinedStats] Error reading first half stats for ${matchId}:`, error);
            return null;
        }
        finally {
            client.release();
     ...
     */
    async hasFirstHalfStats(matchId) {
        const client = await connection_1.pool.connect();
        try {
            const result = await client.query(`
     ...
     */
    async getSecondHalfStats(matchId) {
        const client = await connection_1.pool.connect();
        try {
            ...
            logger_1.logger.error(`[CombinedStats] Error reading second half stats for ${matchId}:`, error);
            return null;
        }
        finally {
            client.release();
