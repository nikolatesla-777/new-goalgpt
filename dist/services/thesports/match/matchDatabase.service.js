...
const matchRecent_service_1 = require("./matchRecent.service");
const matchMinuteText_1 = require("../../../utils/matchMinuteText");
const liveMatchCache_service_1 = require("./liveMatchCache.service");
class MatchDatabaseService {
     ...
     */
    async getMatchesByDate(date, statusFilter, includeAI = false) {
        try {
            // Convert date format to YYYYMMDD if needed
            let dateStr = date.replace(/-/g, '');
            if (!/^\d{8}$/.test(dateStr)) {
                logger_1.logger.error(`Invalid date format: ${date}. Expected YYYYMMDD or YYYY-MM-DD`);
            ...
            logger_1.logger.info(`🔍 [MatchDatabase] Querying matches for date ${dateStr} (${startUnix} - ${endUnix})`);
      ...
      `;
            ...
            if (statusFilter && statusFilter.length > 0) {
                ...
                logger_1.logger.info(`🔍 [MatchDatabase] Filtering by status: ${statusFilter.join(', ')}`);
            }
            query += ` ORDER BY m.match_time ASC, c.name ASC`;
            const result = await connection_1.pool.query(query, params);
            const matches = result.rows || [];
            logger_1.logger.info(`✅ [MatchDatabase] Found ${matches.length} matches in database for date ${dateStr}`);
            // Transform database rows to match API response format
            const transformedMatches = matches.map((row) => {
                ...
                const minuteText = (0, matchMinuteText_1.generateMinuteText)(minute, validatedStatus);
            ...
            logger_1.logger.error(`❌ [MatchDatabase] Error querying matches:`, error);
            return {
                results: [],
     ...
     * @param includeAI Optional flag to include AI predictions (default: false for backward compatibility)
     */
    async getLiveMatches(includeAI = false) {
        try {
        ...
        ORDER BY
          ...
          m.match_time DESC
      `;
            const result = await connection_1.pool.query(query, [nowTs]);
            const matches = result.rows || [];
            // #region agent log
            ...
            // #endregion
            logger_1.logger.info(`✅ [MatchDatabase] Found ${matches.length} strictly live matches in database (status_id IN 2,3,4,5,7, NO TIME WINDOW)`);
            ...
            */
            ...
            logger_1.logger.debug(`[MatchDatabase] Cache SET - ${transformedMatches.length} live matches`);
            return response;
        }
        catch (error) {
            logger_1.logger.error(`❌ [MatchDatabase] Error querying live matches from database:`, error);
            return {
                results: [],
                err: error.message || 'Database query failed',
            };
     ...
     */
    async getShouldBeLiveMatches(maxMinutesAgo = 120, limit = 200) {
        try {
        ...
        LIMIT $3
      `;
            const result = await connection_1.pool.query(query, [now, effectiveMinTime, safeLimit]);
            const matches = result.rows || [];
            logger_1.logger.info(`🔍 [MatchDatabase] Found ${matches.length} should-be-live matches (status=1, match_time passed, window=${safeMaxMinutesAgo}m)`);
            ...
            logger_1.logger.error(`❌ [MatchDatabase] Error querying should-be-live matches:`, error);
            return {
                results: [],
                err: error.message || 'Database query failed',
            };
