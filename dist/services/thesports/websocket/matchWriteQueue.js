...
exports.MatchWriteQueue = void 0;
...
class MatchWriteQueue {
    constructor() {
        ...
        this.batchSize = 10; // Flush when queue has 10 matches
    ...
    enqueue(matchId, update) {
        ...
        if (this.queue.size >= this.batchSize) {
            this.flush().catch(err => {
                logger_1.logger.error('[MatchWriteQueue] Error flushing on batch size:', err);
            });
        }
    }
    /**
    ...
    startFlushInterval() {
        this.flushInterval = setInterval(() => {
            if (this.queue.size > 0 && !this.isFlushing) {
                this.flush().catch(err => {
                    logger_1.logger.error('[MatchWriteQueue] Error flushing on interval:', err);
                });
            }
    ...
    async flush() {
        ...
        try {
            ...
            logger_1.logger.debug(`[MatchWriteQueue] Flushed ${batches.length} match updates`);
        }
        catch (error) {
            logger_1.logger.error('[MatchWriteQueue] Error in batch write:', error);
        ...
        try {
            ...
            for (const batch of batches) {
                try {
                    ...
                    logger_1.logger.error(`[MatchWriteQueue] Error writing batch for match ${batch.matchId}:`, error);
     ...
     */
    async writeBatch(client, batch) {
        // Optimistic locking check
        const freshnessCheck = await this.shouldApplyUpdate(client, batch.matchId, batch.providerUpdateTime);
        if (!freshnessCheck.apply) {
            logger_1.logger.debug(`[MatchWriteQueue] Skipping stale update for ${batch.matchId}`);
            return;
        }
    ...
    `;
        try {
            ...
            if (result.rowCount === 0) {
                logger_1.logger.warn(`[MatchWriteQueue] UPDATE affected 0 rows for match ${batch.matchId}`);
            }
        }
        catch (error) {
            // Enhanced error logging for debugging (only log detailed info on actual errors)
            logger_1.logger.error(`[MatchWriteQueue] Error writing batch for match ${batch.matchId}:`, error.message || error);
            if (process.env.NODE_ENV === 'development') {
                logger_1.logger.error(`[MatchWriteQueue] Query: ${query}`);
                logger_1.logger.error(`[MatchWriteQueue] Values: ${JSON.stringify(values.map((v, i) => ({ index: i + 1, type: typeof v, value: v })))}`);
                logger_1.logger.error(`[MatchWriteQueue] SetParts: ${JSON.stringify(setParts)}`);
            }
    ...
    async shouldApplyUpdate(client, matchId, incomingProviderUpdateTime) {
        ...
        if (result.rows.length === 0) {
            logger_1.logger.warn(`[MatchWriteQueue] Match ${matchId} not found in DB during optimistic locking check`);
    ...
    stop() {
        ...
        this.flush().catch(err => {
            logger_1.logger.error('[MatchWriteQueue] Error flushing on stop:', err);
        });
    }
}
exports.MatchWriteQueue = MatchWriteQueue;
