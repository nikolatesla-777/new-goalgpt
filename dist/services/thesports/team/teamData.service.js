...
class TeamDataService {
    ...
    async getTeamById(teamId) {
        const cacheKey = `${types_1.CacheKeyPrefix.TheSports}:team:${teamId}`;
        // 1. Check cache
        const cached = await cache_service_1.cacheService.get(cacheKey);
        if (cached) {
            logger_1.logger.debug(`Cache hit for team: ${teamId}`);
            return cached;
        }
        // 2. Check database
        ...
        if (dbTeam) {
            ...
            await cache_service_1.cacheService.set(cacheKey, teamData, types_1.CacheTTL.Day);
            return teamData;
        }
        ...
        try {
            ...
            if (apiTeam) {
                ...
                await cache_service_1.cacheService.set(cacheKey, apiTeam, types_1.CacheTTL.Day);
                return apiTeam;
            }
        }
        catch (error) {
    ...
    async getTeamsByIds(teamIds) {
        ...
        if (missingIds.length > 0) {
            ...
            for (const team of dbTeams) {
                ...
                const cacheKey = `${types_1.CacheKeyPrefix.TheSports}:team:${team.external_id}`;
                await cache_service_1.cacheService.set(cacheKey, teamData, types_1.CacheTTL.Day);
            }
            // 3. Fetch from API for still missing teams
            const stillMissing = missingIds.filter(id => !results.has(id));
            if (stillMissing.length > 0) {
                logger_1.logger.debug(`Fetching ${stillMissing.length} teams from API individually`);
                ...
                const fetchPromises = stillMissing.slice(0, 10).map(async (teamId) => {
                    try {
                        ...
                        logger_1.logger.warn(`Failed to fetch individual team ${teamId} from API:`, error.message);
                    }
                });
    ...
    async enrichFromResultsExtra(resultsExtra) {
        ...
        if (Array.isArray(resultsExtra.team)) {
            ...
            for (const team of resultsExtra.team) {
                if (team.id) {
                    ...
                    // Update cache
                    const cacheKey = `${types_1.CacheKeyPrefix.TheSports}:team:${team.id}`;
                    await cache_service_1.cacheService.set(cacheKey, teamData, types_1.CacheTTL.Day);
                }
            }
        }
        ...
        for (const { external_id, data } of teamsToUpdate) {
            try {
                ...
                logger_1.logger.error(`Failed to update team ${external_id}:`, error);
            }
        }
        logger_1.logger.info(`Enriched ${teamsToUpdate.length} teams from results_extra (format: ${Array.isArray(resultsExtra.team) ? 'ARRAY' : 'OBJECT'})`);
    }
    /**
    ...
    async fetchTeamFromAPI(teamId) {
        try {
            ...
            while (hasMore && page <= maxPages) {
                const response = await this.client.get('/team/additional/list', {
                    page,
                    limit: 100, // Fetch 100 teams per page
                });
                if (response.results && response.results.length > 0) {
                    // Search for the team in current page
                    const team = response.results.find((t) => t.id === teamId);
                    if (team) {
                        return {
                            id: team.id || teamId,
                            name: team.name || null,
            ...
            logger_1.logger.debug(`Team ${teamId} not found in first ${maxPages} pages of /team/additional/list`);
            return null;
        }
        catch (error) {
            logger_1.logger.debug(`Team API fetch failed for ${teamId}:`, error.message);
            return null;
        }
    }
}
