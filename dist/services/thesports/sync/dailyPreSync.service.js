...
const matchAnalysis_service_1 = require("../match/matchAnalysis.service");
const matchLineup_service_1 = require("../match/matchLineup.service");
const standings_service_1 = require("../season/standings.service");
const compensation_service_1 = require("../match/compensation.service");
const tableLive_service_1 = require("../season/tableLive.service");
const logger_1 = require("../../../utils/logger");
const connection_1 = require("../../../database/connection");
const cache_service_1 = require("../../../utils/cache/cache.service");
const types_1 = require("../../../utils/cache/types");
class DailyPreSyncService {
    constructor() {
     ...
     */
    async runPreSync(matchIds, seasonIds) {
        const result = {
            h2hSynced: 0,
            lineupsSynced: 0,
            standingsSynced: 0,
        ...
        logger_1.logger.info(`🔄 Starting pre-sync for ${matchIds.length} matches, ${seasonIds.length} seasons`);
        const BATCH_SIZE = 50;
        ...
        try {
            ...
            while (hasMore && page <= maxPages) {
                ...
                logger_1.logger.info(`🔄 [H2H] Fetched compensation page ${page}: ${results.length} matches (total: ${compensationMap.size})`);
            ...
            logger_1.logger.info(`✅ [H2H] Fetched ${compensationMap.size} matches from compensation/list`);
            // Now sync H2H for each match using the map
            for (let i = 0; i < matchIds.length; i += BATCH_SIZE) {
                const batch = matchIds.slice(i, i + BATCH_SIZE);
                logger_1.logger.info(`🔄 [H2H] Syncing H2H batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(matchIds.length / BATCH_SIZE)} (${batch.length} matches)`);
                for (const matchId of batch) {
                    try {
                        ...
                        if (foundMatch) {
                            ...
                            logger_1.logger.debug(`[H2H] Match ${matchId} not found in compensation/list, trying /match/analysis`);
            ...
            logger_1.logger.error(`❌ [H2H] Failed to fetch compensation/list: ${error.message}`);
        ...
        for (let i = 0; i < matchIds.length; i += BATCH_SIZE) {
            const batch = matchIds.slice(i, i + BATCH_SIZE);
            logger_1.logger.info(`🔄 Syncing Lineups batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(matchIds.length / BATCH_SIZE)} (${batch.length} matches)`);
            for (const matchId of batch) {
                try {
        ...
        for (let i = 0; i < uniqueSeasons.length; i += BATCH_SIZE) {
            ...
            logger_1.logger.info(`🔄 Syncing Standings batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(uniqueSeasons.length / BATCH_SIZE)} (${batch.length} seasons)`);
        ...
        logger_1.logger.info(`✅ Pre-sync complete: H2H=${result.h2hSynced}, Lineups=${result.lineupsSynced}, Standings=${result.standingsSynced}, Compensation=${result.compensationSynced}`);
        return result;
    }
    /**
     ...
     */
    async syncH2HToDb(matchId) {
        try {
            logger_1.logger.info(`[syncH2HToDb] Starting sync for match ${matchId}`);
            ...
            while (page <= maxPages && !foundMatch) {
                ...
                foundMatch = compensationResponse.results.find((m) => m.id === matchId);
                if (foundMatch) {
                    logger_1.logger.info(`[syncH2HToDb] Found match ${matchId} in compensation/list page ${page}`);
                    break;
                }
                // If less than 100 results, we've reached the end
            ...
            if (!foundMatch) {
                logger_1.logger.info(`[syncH2HToDb] Match ${matchId} not found in compensation/list, trying /match/analysis as fallback`);
                // CRITICAL: Delete cache to force fresh API call
                const cacheKey = `${types_1.CacheKeyPrefix.TheSports}:match:analysis:${matchId}`;
                ...
                if (!results || Object.keys(results).length === 0) {
                    logger_1.logger.warn(`[syncH2HToDb] No H2H data for match ${matchId} - both endpoints returned empty`);
                    return false;
                }
            ...
            const h2hMatches = []; // Compensation endpoint doesn't provide individual match list
            ...
            logger_1.logger.info(`[syncH2HToDb] Parsed data from compensation/list for ${matchId}: history=${Object.keys(history).length > 0 ? 'yes' : 'no'}, recent=${Object.keys(recent).length > 0 ? 'yes' : 'no'}`);
            return await this.saveH2HToDatabase(matchId, h2hMatches, homeRecentForm, awayRecentForm, goalDistribution, foundMatch, history);
        }
        catch (error) {
            logger_1.logger.error(`❌ Failed to sync H2H for match ${matchId}: ${error.message}`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async syncH2HFromCompensationData(matchId, foundMatch) {
        try {
            const history = foundMatch.history || {};
            const recent = foundMatch.recent || {};
            if (!history.home || !history.away) {
                logger_1.logger.debug(`[syncH2HFromCompensationData] No history data for ${matchId}`);
                return false;
            }
            // Extract H2H data
            const h2hMatches = []; // Compensation doesn't provide individual match list
            const homeRecentForm = recent.home ? [recent.home] : [];
            const awayRecentForm = recent.away ? [recent.away] : [];
            ...
            logger_1.logger.error(`❌ Failed to sync H2H from compensation data for ${matchId}: ${error.message}`);
            return false;
        }
    }
    /**
     ...
     */
    async saveH2HToDatabase(matchId, h2hMatches, homeRecentForm, awayRecentForm, goalDistribution, rawResponse, history) {
        ...
        if (totalMatches === 0) {
            logger_1.logger.warn(`[syncH2HToDb] No H2H matches found for ${matchId}`);
            return false;
        }
        ...
        `, [
            ...
            logger_1.logger.info(`✅ Synced H2H for match ${matchId}: ${totalMatches} matches (${homeWins}H-${draws}D-${awayWins}A)`);
            return true;
        }
        finally {
            client.release();
     ...
     * @returns true if data was synced, false if no data available
     */
    async syncLineupToDb(matchId) {
        try {
            const response = await this.matchLineupService.getMatchLineup({ match_id: matchId });
            const results = response.results || {};
            if (!results || Object.keys(results).length === 0) {
                logger_1.logger.debug(`No lineup data for match ${matchId}`);
                return false;
            }
        ...
        `, [
                ...
                logger_1.logger.debug(`✅ Synced lineup for match ${matchId}`);
                return true;
            }
            finally {
                client.release();
            ...
            logger_1.logger.warn(`Failed to sync lineup for match ${matchId}: ${error.message}`);
    ...
    async syncStandingsToDb(seasonId) {
        try {
            ...
            logger_1.logger.warn(`Failed to sync standings for season ${seasonId}: ${error.message}`);
            return false;
        }
    }
    /**
     ...
     */
    async getH2HFromDb(matchId) {
        const client = await connection_1.pool.connect();
        try {
            const result = await client.query('SELECT * FROM ts_match_h2h WHERE match_id = $1', [matchId]);
     ...
     */
    async getLineupFromDb(matchId) {
        const client = await connection_1.pool.connect();
        try {
            const result = await client.query('SELECT * FROM ts_match_lineups WHERE match_id = $1', [matchId]);
            return result.rows[0] || null;
        }
        finally {
            client.release();
    ...
    async getStandingsFromDb(seasonId) {
        const client = await connection_1.pool.connect();
        try {
            const result = await client.query('SELECT * FROM ts_standings WHERE season_id = $1', [seasonId]);
            return result.rows[0] || null;
        }
        finally {
            client.release();
