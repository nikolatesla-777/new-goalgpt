...
exports.getMemberActivityLogs = getMemberActivityLogs;
...
async function getMemberDetail(userId) {
    ...
    `, [user.id]);
        ...
        logger_1.logger.error('Error fetching member detail:', error);
        throw error;
    }
    finally {
        client.release();
...
async function getMemberActivityLogs(userId, limit = 50) {
    const client = await connection_1.pool.connect();
    try {
        // Get user ID first
        const userResult = await client.query(`
    ...
    `, [internalUserId, limit]);
        ...
        logger_1.logger.error('Error fetching member activity logs:', error);
        return [];
    }
    finally {
        client.release();
