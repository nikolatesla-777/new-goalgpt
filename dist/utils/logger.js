...
const consoleFormat = process.env.NODE_ENV === 'production'
    ? winston_1.default.format.combine(winston_1.default.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), winston_1.default.format.printf(({ timestamp, level, message, ...meta }) => `${timestamp} [${level.toUpperCase()}]: ${message} ${Object.keys(meta).length && !meta.service ? JSON.stringify(meta) : ''}`))
