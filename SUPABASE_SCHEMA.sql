-- ============================================
-- GoalGPT Database Schema for Supabase
-- ============================================
-- Date: 24 Aralık 2025
-- Project: nikolatesla-777's Project
-- Supabase URL: https://wakbsxzocfpngywyzdml.supabase.co
-- ============================================

-- Enable UUID extension (Supabase has this by default, but safe to include)
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- ============================================
-- CUSTOMER & SUBSCRIPTION TABLES
-- ============================================
...
-- ============================================
-- TS MATCHES TABLE (Main Table with All Migrations)
-- ============================================

-- TS Matches Table (Base Schema)
...
-- Add Score Columns (Migration: add-score-columns-to-ts-matches)
...
-- Add Live Match Columns (Migration: add-match-live-columns)
ALTER TABLE ts_matches
...
-- Add Phase 3 Live Columns (Migration: add-phase3-live-columns)
ALTER TABLE ts_matches
...
-- Add Statistics Column (Migration: add-statistics-column-to-ts-matches)
ALTER TABLE ts_matches
  ADD COLUMN IF NOT EXISTS statistics JSONB;

-- Add Incidents Column (Migration: add-incidents-column-to-ts-matches)
ALTER TABLE ts_matches
  ADD COLUMN IF NOT EXISTS incidents JSONB;

-- TS Match Live Data Table
...
CREATE INDEX IF NOT EXISTS idx_ts_matches_score_display ON ts_matches(home_score_display, away_score_display);
CREATE INDEX IF NOT EXISTS idx_ts_matches_live_kickoff ON ts_matches(live_kickoff_time) WHERE live_kickoff_time IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_ts_matches_first_half_kickoff ON ts_matches(first_half_kickoff_ts) WHERE first_half_kickoff_ts IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_ts_matches_second_half_kickoff ON ts_matches(second_half_kickoff_ts) WHERE second_half_kickoff_ts IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_ts_matches_overtime_kickoff ON ts_matches(overtime_kickoff_ts) WHERE overtime_kickoff_ts IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_ts_matches_provider_update_time ON ts_matches(provider_update_time) WHERE provider_update_time IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_ts_matches_last_event_ts ON ts_matches(last_event_ts) WHERE last_event_ts IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_ts_matches_statistics ON ts_matches USING GIN (statistics);
CREATE INDEX IF NOT EXISTS idx_ts_matches_incidents ON ts_matches USING GIN (incidents);
